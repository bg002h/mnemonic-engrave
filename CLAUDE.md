...
- **R0 gate — NO code before GREEN (0C/0I), for RISK-SET work.** Brainstorm specs and implementation plan-docs for work in the risk set above MUST pass an architect R0 review and converge to 0 Critical / 0 Important before implementation begins. Persist the review verbatim to `design/agent-reports/` → fold findings → **run the build gate** → re-dispatch until GREEN. Proceeding past a gate (start coding, advance phase, tag, ship) with an open Critical/Important is prohibited.
  ...
  **A PLAN'S GREEN EXPIRES. Review each phase plan IMMEDIATELY BEFORE dispatching its implementer, never once and early (user directive 2026-08-27: "completion of a prior phase implementation may make plan stale").** A GREEN earned against a tree that has since moved is a claim about a tree nobody is building on. Banking it early is how a reviewed plan sends an implementer at code that no longer exists.
  Measured the same day, on this cycle: **14 of 15 `mt` line citations in the P1 plan went stale under P1's OWN first four rows** — and `plan-cite-check.sh` reported 41/41 resolved, 0 dangling, because it checks that a line *exists* and never what is on it. One cited line had come to hold a real function the plan cites elsewhere: plausible, wrong, and invisible to every gate. Separately, the P2 and P3 plans were both written against a crate that P1's rows 5 and 6 then changed underneath them.
  **So the order is: prior phase closes green → THEN re-validate the next plan → THEN dispatch.** The re-validation is not a fresh audit and is usually cheap: run `scripts/plan-staleness-check.sh <plan> <repo> <baseline-rev>` to find drifted citations mechanically, re-run the three plan gates, and scope any re-review to *"what did the last phase falsify here?"* — the [[a-diff-falsifies-text-it-never-touches]] lens, not a re-read. Only a *material* change earns a full round. **Record each plan's baseline revision in the plan itself**, or the staleness check has nothing to compare against.
