...
- **R0 gate — NO code before GREEN (0C/0I), for RISK-SET work.** Brainstorm specs and implementation plan-docs for work in the risk set above MUST pass an architect R0 review and converge to 0 Critical / 0 Important before implementation begins. Persist the review verbatim to `design/agent-reports/` → fold findings → **run the build gate** → re-dispatch until GREEN. Proceeding past a gate (start coding, advance phase, tag, ship) with an open Critical/Important is prohibited.
  ...
  **A PLAN'S GREEN EXPIRES — review each phase plan IMMEDIATELY BEFORE dispatching its implementer, not once and early (user directive 2026-08-27: "completion of a prior phase implementation may make plan stale").** A GREEN earned against a tree that has since moved is a claim about a tree nobody is building on. So the order is: prior phase closes green → re-validate the next plan → dispatch. Scope the re-validation to *"what did the last phase falsify here?"* — not a fresh audit, and not a measurement exercise. `scripts/plan-staleness-check.sh <plan> <repo> <baseline-rev>` finds drifted citations mechanically if you want it; record each plan's baseline revision in the plan so it has something to compare against.
