...
run "$MD" encode --group-size 0 "$(cat "$W/inputs/wallet-policy.txt")"
...
run "$MK" encode --xpub "$XPUB" --origin-fingerprint ae6647ee \
...
run "$MS" encode --phrase "$(cat "$W/inputs/seeds/cosigner-00.seed")"
