#!/usr/bin/env python3
"""Capture the Wallet Policy journey's device half, and CHECK it against the host.
...
It is a comparison, not a photo shoot. cmd/emu/shots_walletpolicy.js is handed
...
implementations, in two languages, from the same four xpubs.
    ...
    python3 capture_walletpolicy.py [--port 8793] [--shot-port 8734]
...
EXPECTED = ["w00-boot.png", "w01-carousel.png", "w02-gather-empty.png",
            "w03-gather-full.png", "w04-consent-p0.png"]
...
def read_artifacts():
    ...
    def need(name):
        ...
        if not os.path.exists(p):
            sys.exit(f"missing {p}\nRun ./transcript_walletpolicy.sh first -- it writes this.")
        return [l.strip() for l in open(p) if l.strip()]

    md1 = need("walletpolicy.md1.txt")
    ids = need("walletpolicy.id.txt")
    recv = need("walletpolicy.receive.txt")
    chg = need("walletpolicy.change.txt")
    ...
    if not policy_id:
        sys.exit("no wallet-policy-id in walletpolicy.id.txt")
...
def main():
    ...
    ap.add_argument("--shot-port", type=int, default=8734)
    ...
    with open(os.path.join(SHOTS, "walletpolicy-result.json"), "w") as f:
