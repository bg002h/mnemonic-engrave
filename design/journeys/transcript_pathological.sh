...
T=$(cat "$W/inputs/wallet-policy.txt")
MD1S=$(tr '\n' ' ' < "$W/out/md1.txt")
...
XPUB=$(grep '^xpub' "$W/inputs/keys/key-00.xpub")
...
run "$ME" bundle --in "$W/inputs/backup-strings.txt" --preview "$W/out/plates" --png --manifest "$W/out/manifest.json"
...
run cat "$W/inputs/seeds/master-A.seed"
run "$MS" encode --phrase "$(cat "$W/inputs/seeds/master-A.seed")"
MS1=$("$MS" encode --phrase "$(cat "$W/inputs/seeds/master-A.seed")" --no-engraving-card 2>/dev/null | tr -d ' ')
run "$ME" --in <(printf '%s\n' "$MS1") --hex
