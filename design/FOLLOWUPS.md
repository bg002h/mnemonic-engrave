...
RULED 2026-09-03 (operator): `ms` owns it. Rust first.
...
RULED 2026-09-03 (operator, verbatim): "We must allow user to type hashlock phrase on the SH2 also, but the code should be rust first." So this is a CHANGE REQUEST, not a question: the hashlock-phrase -> preimage -> digest rule lands in Rust (`ms hashlock`, F-465, with test vectors) and the device's entry screen is its Go port, behaviour-faithful, under the Rust-primary rule; the spec folds (§6c, §14 row, C25) get their own R0 in the cycle that builds it. Owning phase: the next composer cycle (after S4 closes).

BRAINSTORMED 2026-09-03: `design/BRAINSTORM_hashlock_phrase.md` (rulings L1-L9: two methods, hardened = PBKDF2-HMAC-SHA256 100,000 / `sha256`; a new ms1 kind byte for the preimage; device = digest only).
