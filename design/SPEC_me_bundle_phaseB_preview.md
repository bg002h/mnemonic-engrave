...
## 2. Scope & non-goals
- **In scope (v0.3.0):** the `me-preview` Go sidecar (render one validated public string → SVG, optional PNG); `me bundle --preview <dir>` integration (drive the sidecar per public plate; `--version` staleness check; graceful degrade); the fidelity contract (replicated SH2 params + exact B-spline strokes).
- **Deferred to a fast-follow (`me-preview-release-ci`):** the **signed cross-platform release-archive automation** (CI matrix + `SHA256SUMS` + signature bundling `me`+`me-preview`). The feature works end-to-end with a locally-built sidecar; the *automated signed release* is operational config best landed separately. This spec defines the runtime contract (`me` finds `me-preview` beside itself / on `$PATH`, version-checked) the release archive must satisfy, so the follow-on is purely packaging.
...
- **Dependency pin:** the sidecar's `go.mod` pins **UPSTREAM seedhammer v1.4.2** (`module seedhammer.com`, via `replace seedhammer.com => ../../../seedhammer-ref-v1.4.2`, mirroring `firmware/ndef-roundtrip/go.mod`) + `github.com/seedhammer/kortschak-qr v0.3.2`. It imports `backup`, `engrave`, `bspline`, `bezier`, `font/sh`, `font/vector` — **NOT `gui` and NOT `cmd/controller`** (so it is host-portable and decoupled from PR #35).

## 4. The sidecar `me-preview` (Go)

### 4.1 CLI contract (how `me` invokes it; stable, machine-oriented)
- `me-preview --version` → prints a single line `me-preview <SEMVER>` to stdout, exit 0. `<SEMVER>` MUST equal the `me` crate version it ships with (the lockstep pin).
...
## 7. File structure
- **New** `preview/` Go module in me-repo: `go.mod` (replace upstream v1.4.2 + kortschak-qr), `main.go` (CLI: `--version` / `render`), `params.go` (replicated SH2 `engrave.Params` + plate-fit constants + source citation), `layout.go` (validateMdmk-equivalent: qr.L/scale3/modes/EngraveText/fit-check), `render_svg.go` (PlanEngraving → bspline.Segment → direct cubic-Bézier SVG), `render_png.go` (raster), `*_test.go` (geometry-golden drift guard + render smoke).
...
- Bump `me` → 0.3.0 + CHANGELOG. The **signed cross-platform release archive** (`me` + `me-preview` + `SHA256SUMS` + signature) is the deferred `me-preview-release-ci` follow-on; this spec guarantees the runtime contract it must satisfy (binaries co-located, version-matched).
