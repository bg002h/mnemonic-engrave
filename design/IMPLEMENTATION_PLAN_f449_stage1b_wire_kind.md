...
**Status:** r0 draft. Not yet reviewed.
...
`all_vendored_vector_names` already exists in stage 1a's `tests/common/vendored.rs`
(it carries `#[allow(dead_code)]` there because stage 1a had no caller — this
stage is the caller, so **remove that attribute** when you first use it).
...
- [ ] **Step 3: Implement** — `sha256` over each leaf's 33-byte compressed pubkey in **descriptor left-to-right order**, which is **wire order, never the derived-key-sorted order** the device's address builder uses for `sortedmulti_a`; pubkey = the BIP-341 NUMS point compressed; depth 0, parent 0, child 0; network from the parameter. **`hex_lit` is not a dependency** — use a byte array or the `bitcoin` crate already in the graph.
...
- [ ] **Step 4: Switch the THREE production callers** (G-7): `cmd/descriptor.rs` (has `args.network` at `:62`), **`derive.rs:134`** (inside `derive_address`, which already has a `network` parameter in scope — miss this and every kind-1 address derivation returns `Err`, disabling Task 8's address gate), and **`cmd/vectors.rs:193`**.
...
| the template grammar | n/a | accept `UNSPENDABLE(liana)` via a substitution rule in **`substitute_synthetic`**, NOT `walk_tr` — `walk_tr` runs after `Descriptor::from_str`, which fails first on a non-key token |
| `md encode` with a literal xpub | **no** | refuse, naming both working spellings |

`md encode` cannot recompute: `substitute_synthetic` replaces every `@i` with a `sha256(b"md-v0.15" ‖ i ‖ depth)` placeholder **before** `walk_tr` sees the tree, so the leaves are synthetic and a match can never succeed. Today it leaks `"internal: synthetic key … not found in key map"` — replace that, do not keep it.
...
## Task 7: §6's refusals — ENCODE-SIDE ONLY

**Files:** `validate.rs`, `error.rs`.

**Fixture this task defines**, used by its own test and by Task 9's mutations:

```rust
/// A kind-1 tr whose taptree contains a `sortedmulti_a` leaf — the shape §6
/// row 1 refuses. Built from the vendored single-path bare-multi vector, whose
/// sole leaf IS a sortedmulti_a, then switched to kind 1.
fn tr_liana_with_sortedmulti_a_leaf() -> Descriptor {
    kind1_from_vector("keyed_tr_sortedmulti_a")
}
```

Four refusals: a `sortedmulti_a` leaf anywhere (a belt against the port error — §2 hashes account-level pubkeys, which are index-independent, while `MultiALeafScript` sorts **derived** keys); a use-site other than `<0;1>`; kind 1 nested under `sh`/`wsh`; and version 8 with no kind-1 node (refused at encode only).

**They hook the encode/admission path and MUST NOT reach decode.** Add a test that proves it:

```rust
// UNIT test, inside the crate — Admission is pub(crate), so an integration
// test cannot reach SkipPolicy (G-6).
#[test]
fn a_refused_shape_still_DECODES_so_existing_cards_never_stop_reading() {
    let (bytes, bits) = encode_payload_inner(&tr_liana_with_sortedmulti_a_leaf(),
                                             Admission::SkipPolicy).unwrap();
...
## Task 8: §8's acceptance vectors

Descriptor equality **including the BIP-380 checksum** for the four accepted shapes, against `cases.json`; three-way address equality (md vs the evidence's recorded Liana addresses vs kind 0, which must **differ**); identity distinctness (different `WalletPolicyId`, `WalletDescriptorTemplateId` and 12-word phrase) **and** stability (every existing v4 id byte-preserved, against stage 1a's committed `pre_refactor_ids.json`); the dispatch round trip through `decode_with_correction`, single-string and chunked; and the render-reparse fixpoint extended to `tr` kind 1.

The kind-1 chunks are **constructed**, not vendored — `chunk::split(&kind1_from_vector(name))` (G-4). No card of this kind exists anywhere yet; this stage creates the first.

---

## Task 9: §8.10's mutation pass

Each mutation is applied to the source, the suite is run, and the result recorded in `design/agent-reports/f449-stage1b-mutations.md`. **A mutation that leaves the suite green is a missing test.** Before each, confirm the mutated line actually ran.

| # | mutation | must be caught by |
| --- | --- | --- |
| 1 | reverse the concat order | Task 4 golden |
| 2 | `sort()` the leaves | Task 4 sort/dedup |
| 3 | `dedup()` the leaves | Task 4 sort/dedup |
| 4 | `wire_version()` returns 4 always | Task 3 round trip + Task 8 identity |
| 5 | render kind 1 as the NUMS hex | Task 5 + Task 8 descriptor equality |
| 6 | a constant 4 at `identity.rs:90`/`:200` | **Task 8 identity distinctness — the funds-relevant one** |
| 7 | write the kind bit at version 4 | **stage 1a's byte-equality gate** |
| 8-11 | drop each of §6's four refusals | Task 7 |
| 12-15 | **collapse each of G-1's four or-pattern sites to the NUMS arm** | Task 5 — if any stays green, that site has no gate |
