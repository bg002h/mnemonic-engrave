...
- [ ] **Step 2: Fuzz** `FuzzEncodeSingleSig` (random valid (xpub,fp,origin,script) → encode → `DecodeChunks`→`ExpandWalletPolicyChunks` recovers the inputs; no panic), `FuzzEncodeMS1` (random entropy → `DecodeMS1` round-trips; no panic), and the comparator (no panic). ≥1M execs each.
- [ ] **Step 3: Run → 0 panics.**
...
- Spec Phase-A coverage: `EncodeSingleSig`→T2; `EncodeMS1`→T1; comparator→T3; differential goldens→T0. Spec-R0 folds: C1/C2/C3 wire→T2; C4 ms1→T1; spec-I2 signature→T2; restore/scrub→T6a-2 (GUI).
