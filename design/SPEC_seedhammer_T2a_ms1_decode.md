...
7. **Offer decode ONLY for an UNSHARED secret that is ACTUALLY a decodable m-format ms1 (refined by plan-R0 C-1).** `engraveCodex32`/`confirmCodex32Flow` handle three codex32.String shapes: the unshared secret (`Fields.Unshared`, index `s`), a K-of-N **share** (index ≠ S — carries an SSS-evaluated point, NOT an m-format payload), and a recovered secret. **AND** not every unshared codex32 secret is an m-format ms1: a plain **BIP-93** codex32 secret (e.g. the upstream `ms10test…` vector) holds a raw master seed whose `Seed()[0]` is not a valid m-format prefix, so `DecodeMS1` correctly refuses it. So the "Show secret" affordance is offered **only when `f.Unshared` is true AND `DecodeMS1(scan)` succeeds** — probed once (confirmCodex32Flow is not alloc-gated). For a non-decodable unshared secret the affordance is **hidden and Button2 stays inert** (exactly as today — preserving `TestConfirmCodex32UnsharedNoRecover`); shares still get Recover on Button2. This both avoids a "can't decode" message on a button-press and keeps the affordance honest. (A raw share never reaches `DecodeMS1`; the gate prevents it.)
