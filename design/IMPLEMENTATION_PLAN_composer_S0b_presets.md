...
**STATUS: R0 GREEN 2026-09-02 (0 Critical / 0 Important open).** Round 0: fidelity+design (opus, `composer-S0b-plan-R0-r0-fidelity.md`, 0C/4I/4M/2N) and tests/mutation (sonnet, `composer-S0b-plan-R0-r0-tests.md`, 0C/0I/0M/2N, 12/12 mutations caught), folded at 6c308b6; round 1 verification (sonnet, `composer-S0b-plan-R0-r1-fold-verification.md`, 0C/1I/1M: a tautological drift test) folded at b2c72f5; round 2 verification (sonnet, `composer-S0b-plan-R0-r2-fold-verification.md`, 0C/1I: one refusal-table row still carried the superseded "is read as" wording) folded by the controller as a one-word change, propagation-checked (`grep -c "is read as"` = 0) and not re-dispatched under the proportional re-review rule. **This GREEN expires:** re-validate against descriptor-mnemonic main immediately before dispatching the implementer (baseline `66bdf2f4`).
...
| `decaying-multisig`'s `after=` at or above the Unix-time band | names `--path` as the only remedy, since the preset grammar has no `t` suffix: `` preset <name>: after=<v> reads as a block height and is above the height band (1..=499999999); presets cannot express a Unix time -- use --path with `after=<v>t` instead `` (R0 fidelity M-3) | 1 |
