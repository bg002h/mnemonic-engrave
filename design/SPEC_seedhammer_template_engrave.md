...
- **Status:** DRAFT v3 — md-codec readiness folded (v2) + **R0 round 0 folded (3C/5I → corrected; see Gate)**. Awaiting R0 round 1 (must reach 0C/0I before any plan/code).
...
Inner ChoiceScreen on the existing programs (no new `program` → no `gui/gui.go:164` guard trip). Default lands on full-policy. **I2 placement:** the template opt-in (which triggers the S1 strip) appears on the **single-sig** program and the **multisig on-device BUILD** path; the **multisig SUPPLY** path does NOT strip — a user wanting a template there supplies a keyless template md1, engraved verbatim (S3 form-aware verify binds it). Selecting template-only shows the warning + estimate, then engraves the stripped/supplied template + the N keyless cosigner mk1 stubs. For classifiable shapes (single-sig, `wsh`/`sh(wsh)` multi & sortedmulti) the confirm screen shows full type + k-of-N; for `classifyPolicy`→`PolicyComplex` shapes it shows the honest-minimal consent surface (C3/DD7, second mockup below).
...
- **Open items for R0 to rule on:**
  ...
  - **O4 — STILL OPEN for round 1.** Ratify the recovery-estimate model (sortedmulti→none; ordered/distinct→N!; 6.9/7.4 µs) and the displayed N→time table values.
