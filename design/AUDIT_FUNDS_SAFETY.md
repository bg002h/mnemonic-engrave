...
| Branch protection | DONE 2026-07-06 (user-approved) — master requires status check `test` (strict=false, enforce_admins=false: admin direct pushes still allowed, deliberately paired with the local-merge workflow; PRs and non-admin pushes are gated) | GitHub repo setting via API |
