...
```
(`errors` is already imported by `address_test.go` if not, add it; `nonstandard`/`bip380` already used.)
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			for i := 0; i < 8; i++ {
				...
				click(&ctx.Router, Button1) // dismiss/back through Verifying→result
			}
...
- [ ] **Step 3a: Implement** `gui/verify_address.go` (the result core; Scan/Type input added in Task 3). Mirror `descriptorAddressFlow`'s nav/measure idiom and `showError`:
```go
package gui
...
// runVerify renders a "Verifying…" frame, runs address.Find ONCE (outside any
// frame loop — a multisig gap scan can block for seconds, R0-M3), then shows the
// result. Read-only: no engrave/NFC/mutation.
func runVerify(ctx *Context, th *Colors, desc *bip380.Descriptor, candidate string) {
	showMessage(ctx, th, "Verify address", "Verifying…") // one frame, then compute
	chain, index, found, err := address.Find(desc, candidate, 20)
	var title, body string
	switch {
	case errors.Is(err, address.ErrAddrUnparseable):
		title, body = "Verify address", "Invalid address."
	case errors.Is(err, address.ErrAddrWrongNetwork):
		title, body = "Verify address", "Address is for a different network."
	case err != nil:
		title, body = "Verify address", "Can't verify this address."
	case !found:
		title, body = "Verify address", "Not found in the first 20 receive or change addresses."
	default:
		chainName := "Receive"
		if chain == 1 { chainName = "Change" }
		// Degenerate range/wildcard-less descriptor reports (0,0); phrase plainly.
		body = fmt.Sprintf("Match: %s address #%d.\nControlled by this descriptor.", chainName, index)
		title = "Verify address"
	}
	showMessage(ctx, th, title, body)
}
```
(`showMessage` = a Back-only result screen mirroring `showError`'s structure but neutral/positive; if a suitable helper exists, reuse it — else add a small one here. The "Verifying…" first call renders one frame then returns so `Find` runs; the result call loops until Back.)
...
```
And a scan-recognizer test in `gui/scan_test.go`:
```go
func TestScanRecognizesAddress(t *testing.T) {
	s := new(scanner)
	obj, err := s.Scan(strings.NewReader("bc1qkwl5qpx6k93cqmnygn6kgucgka8q3z4kur2nm8"))
	if err != nil { t.Fatalf("scan: %v", err) }
	if _, isAddr := obj.(addressText); !isAddr {
		t.Fatalf("scanned object = %T, want addressText", obj)
	}
	// A descriptor must NOT be misrecognized as an address.
	obj2, _ := s.Scan(strings.NewReader("wpkh(" + /* a valid xpub */ ")"))
	if _, isAddr := obj2.(addressText); isAddr {
		t.Fatal("descriptor misrecognized as addressText")
	}
}
```
(Use a reader that yields the bytes then EOF — mirror how `scan_test.go`/existing tests drive `scanner.Scan` if a helper exists; otherwise `strings.NewReader` + a loop until non-`errScanInProgress`.)
- [ ] **Step 2: Run — expect FAIL** (`typeAddressFlow`/`addressText` undefined).
- [ ] **Step 3a: Unmasked keyboard** — `gui/passphrase_keyboard.go`: add a constructor (or exported field) yielding a case-preserving keyboard that renders UNMASKED (set `revealed=true` default + drop/disable the `ppReveal` key), e.g. `func NewAddressKeyboard(ctx *Context) *PassphraseKeyboard` returning a `PassphraseKeyboard` with `revealed=true`. The `Layout` masking branch (`:341-344`) then shows `k.Fragment` directly. (Minimal change; keep `PassphraseKeyboard` otherwise intact so `passphraseFlow` is unaffected.)
...
- [ ] **Step 3c: scan recognizer** — `gui/scan.go`: add `type addressText string` and, in `Scan`'s dispatch chain (before the final `else`), a branch:
```go
	} else if _, aerr := address.DecodeAddress(string(buf), &chaincfg.MainNetParams); aerr == nil {
		return addressText(buf), nil
```
(Import `seedhammer.com/address`? No — `address.DecodeAddress` is the btcd parser, reachable in `gui` via `github.com/btcsuite/btcd/address/v2` + `chaincfg/v2`. Try mainnet then testnet params, or accept any net the descriptor will re-check.) **`engraveObjectFlow` gets NO `addressText` case (R0-M5)** → a top-level address scan falls to `default → unknown format` (unchanged). The `addressText` value is consumed only by the verify flow's scanner-shell.
