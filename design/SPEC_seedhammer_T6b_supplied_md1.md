...
- **Gather the supplied md1** over NFC via the T5 `bundleGatherFlow` (its scan loop accumulates chunked md1), then a **NET-NEW `extractSuppliedMd1(cards) ([]string, ok)`** filter (R0-I1; do NOT reuse `singleSigReadbackCards` — it *requires* BOTH an mk1 AND an md1, `gui/singlesig_verify.go:38`, the opposite of a multisig supply): exactly ONE `cardMD1` → return its verbatim `[]string`; **0 md1 → refuse** ("no wallet policy supplied"); **≥2 md1 → refuse** ("supply exactly one wallet policy"); **any `cardMK1`/`cardMS1` present → refuse** ("supply only the wallet-policy md1"). Then decode via `md.ExpandWalletPolicyChunks` → `Template` + per-@N `[]ExpandedKey`.
