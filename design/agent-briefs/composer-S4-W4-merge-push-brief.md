...
- Branch `composer-s4d` (worktree `/scratch/code/shibboleth/wt-composer-s4d`), tip `bb5077547b422c47084850b119d772c4d9fcc28b`, base `main` = `6fb90cb`. Verify in the main checkout: `git rev-parse main` = 6fb90cb..., `git status --short` empty, `git merge-base --is-ancestor main composer-s4d` true; STOP if not.
- `git merge --no-ff composer-s4d -F /scratch/code/shibboleth/mnemonic-engrave/design/agent-briefs/composer-S4-W4-merge-message.txt` in the main checkout (the merge commit message is given in the dispatch message; keep its trailer lines), then `git push origin main`, then `gh run list --repo bg002h/seedhammer --commit <merge sha> --json databaseId,name,status,conclusion` and `gh run watch <id> --exit-status` in the FOREGROUND on the `test` workflow; judge each job's conclusion.
