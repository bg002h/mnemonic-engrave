...
- Branch `composer-s4d` (worktree `/scratch/code/shibboleth/wt-composer-s4d`), tip `<W4_FIX_SHA>`, base `main` = `6fb90cb`. Verify in the main checkout: `git rev-parse main` = 6fb90cb..., `git status --short` empty, `git merge-base --is-ancestor main composer-s4d` true; STOP if not.
