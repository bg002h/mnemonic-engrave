...
## What to push
- `master` tip must be `203f3bb` (verify `git rev-parse master`; `git status --short` may list untracked files -- this brief is one of them, and a verification report may appear under `design/agent-reports/` while you run -- ignore untracked, but STOP if any tracked file is modified or the tip differs). `origin/master` is `d31d595`, an ancestor; the nine commits between are design records (a push report + brief, brainstorm folds and rulings, two review briefs, a review report, continuity, the walk record) -- no host code.
