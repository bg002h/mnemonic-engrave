...
- Branch `composer-s4b` (worktree `/scratch/code/shibboleth/wt-composer-s4b`), tip `<W2_FIX_SHA>`, base `main` = `60bee002`. Verify in the main checkout: `git rev-parse main` = 60bee002..., `git status --short` empty, `git merge-base --is-ancestor main composer-s4b` true; STOP if not.
