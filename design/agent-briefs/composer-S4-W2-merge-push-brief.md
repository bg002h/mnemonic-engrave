...
- Branch `composer-s4b` (worktree `/scratch/code/shibboleth/wt-composer-s4b`), tip `2dff0ee2cf1824b0381e37fa7a1fccb739393157`, base `main` = `60bee002`. Verify in the main checkout: `git rev-parse main` = 60bee002..., `git status --short` empty, `git merge-base --is-ancestor main composer-s4b` true; STOP if not.
- `git merge --no-ff composer-s4b -F /scratch/code/shibboleth/mnemonic-engrave/design/agent-briefs/composer-S4-W2-merge-message.txt` in the main checkout (the merge commit message is given in the dispatch message; keep its trailer lines), then `git push origin main`, then `gh run list --repo bg002h/seedhammer --commit <merge sha> --json databaseId,name,status,conclusion` and `gh run watch <id> --exit-status` in the FOREGROUND on the `test` workflow; judge each job's conclusion.
