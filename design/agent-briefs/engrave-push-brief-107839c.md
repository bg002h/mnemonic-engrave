...
## What to push
- `master` tip must be `107839cbd35d0700253e6320d263876515ef95f7` (verify `git rev-parse master`; `git status --short` may list untracked files -- this brief is one of them -- ignore untracked, but STOP if any tracked file is modified or the tip differs). `origin/master` is `3d55279`, an ancestor; the four commits between are DESIGN RECORDS ONLY -- no host code changed. They are the S4 plan's firmware pin fold, the fork push report, the previous push report and brief, and continuity.
