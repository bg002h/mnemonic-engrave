...
- Branch `composer-s4-emu` (worktree `/scratch/code/shibboleth/wt-engrave-s4-emu`), tip `<S4_FOLD_ENGRAVE_TIP>`, base `master` `a262e7d`. In the main checkout: `git rev-parse master` = `<ENGRAVE_MASTER_TIP>`, `git status --short` empty (ignore untracked), `git merge-base --is-ancestor a262e7d composer-s4-emu` true; STOP if not.
