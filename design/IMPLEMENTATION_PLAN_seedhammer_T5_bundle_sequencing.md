...
- [ ] **Step 3 (R0-M1 — fixtures, corrected):** In a test helper (`gui/bundle_testdata_test.go`), assemble complete, distinct-csid chunk sets from REACHABLE sources (the `md` `split` helper is unexported / cross-package-unreachable from `gui` — do NOT rely on it):
  - **mk1 (two distinct cards):** `mk.Encode(card)` is fully reachable. Mint two distinct csids by varying `Stubs`/`Fingerprint` between the two cards (NOT the path — the depth/child invariant constrains it). Confirm each set: every chunk `ValidMK`, `mk.Decode` round-trips.
  - **md1 (two distinct cards):** use the two chunked-md1 sets already reachable from gui tests — `wshSortedmultiChunks` (csid `0x2d950`, the constant embedded in `gui/md1_gather_test.go`) and `wsh_multi_chunked` (csid `0x157ae`, via `loadChunkedVectorString`). Confirm each `md.DecodeChunks` round-trips.
  - (R0-M2 note for Task 4: the real `multiPlateEngrave` call site is `gui/derive_xpub.go:162`; `:269/:300-301` are interior copy strings.)
...
- [ ] **Step 2: Run → FAIL.**
- [ ] **Step 3: Implement** — R0-M2: Go has NO default params, so the "add optional params" option is NOT viable for keeping `deriveXpubFlow`'s call site (`gui/derive_xpub.go:162`) byte-unchanged. Use ONE of: (i) a thin `bundleEngrave(ctx, th, cards)` sequencer ALONGSIDE `multiPlateEngrave`, reusing the same per-plate `validateMdmk` machinery (PREFERRED — zero touch to the existing helper); or (ii) extract a shared lower-level helper that BOTH `multiPlateEngrave` (unchanged signature/behavior) and `bundleEngrave` call. Either way `deriveXpubFlow`'s call site stays byte-unchanged (R0-I-C, I-7). Loop cards → per-card plates "Card X of Y · Plate P of Q"; set-level abort (extend `abortWarning`) records no completed state; append the ms1 reminder (mirror host `bundle.rs:296-306`).
- [ ] **Step 4: Run → PASS.**
