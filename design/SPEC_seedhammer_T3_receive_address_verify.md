...
6. **2.6 No regression (with one intended T1 interaction change).** The engrave path and all prior cycles (T2a/T2b/T2c) are behaviorally unchanged. The T1 **show-addresses** view remains reachable and identical, but is now entered via a Button2 **"Show addresses" / "Verify an address" `ChoiceScreen`** (one extra selection — intended per §4.2, the only feasible wiring since no Button4 exists). The existing `TestDescriptorConfirmAddressAffordance` MUST be updated to drive through this choice (select "Show addresses"). The verify affordance is additive and gated on `address.Supported`; the per-frame layout + alloc gate are undisturbed (the ChoiceScreen lives inside the click branch).
...
- **Modify** `gui/gui.go` (`DescriptorScreen.Confirm`/`descriptorAddressFlow`) — the verify affordance via a Button2 **Show-addresses / Verify-address `ChoiceScreen`** (R0-I3; the only feasible route — no Button4), inside the existing `addrBtn.Clicked && supported` click branch so the per-frame layout + alloc gate are undisturbed. (R0-M7: the plan should note `Find` returns the new exported `ErrUnsupported` for the keyless guard while keyed-but-unsupported surfaces via the propagated wrapped `errUnsupported` — two distinct sentinels, no collision; the GUI only reaches `Find` for `Supported` descriptors.)
