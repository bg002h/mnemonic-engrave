...
- **No-skip paging (the load-bearing regression test):** page forward (repeated Button3) across enough pages to cover indices `0..7` and assert **every** `address.Receive(desc,i)` for `i∈0..7` appears in some rendered frame — for BOTH a single-sig fixture (more fit per page) AND a long-address P2WSH fixture (fewer fit per page), since the fixed-page bug only manifested on the long-address case. This proves measure-and-advance never silently drops an index off-screen.
