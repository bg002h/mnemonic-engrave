...
**`-` READS STDIN NOWHERE IN `me` TODAY** — five surfaces, two distinct exit
codes (**2** on `me`, `bundle`, `sysw wipe` and `sysw show`; **4** on `sysw
pack`) — and §6b's permissive wording, which lets an implementation take the
...
it, and justified that by claiming the value scan dominated it — which
§6d's own `--passphrase <arbitrary text>` example refutes: **a flag can carry
...
**`me` DOES leak this way. The table below is a SAMPLE, not the surface** — it
was measured before `--in` and the subcommand shapes were, and the real figure
is larger (round-9 M-1). The gate is a generated cross-product for exactly that
reason:
...
stdio ruling (N-I1).** Two of the closure's four `eprintln!` live in them
(`refuse_terminal_destination` at `crates/me-cli/src/main.rs:1035`, `refuse_world_readable_stdout`
at `:1058`); the other two live in `read_records` (`:2039`, the stdin-tty
prompt) and `emit` (`:2091`, the write-failure report), which stay in `me` for
the same reason — `refuse_write_block` itself contains none. An earlier draft
of this table mapped three of the four into the crate while §3,
...
| 9 | **`exit.rs` FIRST, then `channel.rs`** | **`--out` overwrite is asserted where it LIVES — `write_private`, which stays in `me`** (round-3 M-3); `channel.rs` holds only `destination`, so gating the overwrite on it would gate nothing. **`-` is IMPLEMENTED ON `sysw pack`, the one surface with a positional record list**: `me sysw pack --out b.bin - text:6869`, fed a `text:6162` line on stdin, must exit **0** with the packed container holding **BOTH** records — asserted via `me sysw show b.bin` / unpack, where `pub_len` moves with the record count. **Fails today: exits 4, writes nothing** (measured). The other four surfaces gain nothing this phase and are UNCHANGED, because none declares a positional a `-` could occupy: `me` and `bundle` already default to stdin with no positional at all, `sysw show`'s positional is a container FILE path rather than a record, and `sysw wipe` has no positional — so `me -`, `bundle -` and `sysw wipe -` stay at **rc 2** via clap and `sysw show -` stays at **rc 2** via ENOENT, all measured unchanged. The differential is restated as the exit-probe precedent already is: unchanged everywhere **except the enumerated dash cells above**, each diff justified — not by matching a table |
| 9b | **CREATE `mnemonic-io-lib` and move the lib-half modules into it** (distributing `io.rs` per §3's table). Until here everything lives in `me`'s lib half; this is the crate boundary, and it is a step because no earlier one created it (round-3 M-4). | the crate builds standalone; `me` depends on it by path; **no `EXIT_*` and no `Class` in it** |
...
**M5 — THREE PIECES OF WORK ARE REGRESSION-GATED RATHER THAN RED-FIRST, and the
column header must not claim otherwise.** They are **the signature change**,
**the crate adoption**, and **the dash differential's unchanged-elsewhere
half**. The first two are refactors whose gate is *"the suite still passes"*;
the third is that the four surfaces `-` does not touch stay exactly at their
measured codes, which backstops the RED-first `sysw pack` work rather than
proving it. That is legitimate for a refactor; asserting it is TDD when it is
not would hide which work carries real proof.
