...
**Status (stage 4, fork main merge `2c9eed3` (branch `f449-stage4` at `a66491f`)):** the line numbers above are as of `7b6f2fb`.
...
**Status (stage 4, fork main merge `2c9eed3` (branch `f449-stage4` at `a66491f`)):** item 2 is `taprootInternalKey`'s Liana arm
...
| **5** | rebuild `demo/sh2/` and deploy to quantoshi.xyz/SH2/ with `demo/sh2/update.sh`. **Status:** SHIPPED 2026-09-23 from fork main `2c9eed3`: update.sh verified 200, `application/wasm` and 11,174,471 B; `capture_composer.py --arm liana` run against the LIVE site's own emu files matched the host (the negative control, `--arm keyed --prove-it-can-fail`, caught the corrupted address) | site 200, `application/wasm`, and the emulator reaches §0b's screen |
