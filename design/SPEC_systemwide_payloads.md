...
| D7 (2026-08-12, ruled at fold) | §3.3.2a: ONE admission function on every path | the §3.3.2 table is the normative oracle; enforcement is per-site — each consumption site hard-codes its one admitted class, and a structural test reconciles every site against the table (plan stage 13) | the run-time funnel. `admits()` had zero production callers when this was ruled (one since 2026-08-25: `gui/sysw_unload.go:89`, the Transaction cycle — §3.3.2's dated note), and wired in it could never return false at any existing site — a check that cannot fail — while a wrong future site could omit the call. The RULE (admission is class-only, source-blind) is untouched |
...
| D9 (2026-08-12) | §3.1's source picker precedes EVERY seed entry | it is offered only when a choice actually exists — a loaded payload or a tag source. With neither, entry goes straight to typing | the picker landed on the most-walked path in four programs and cost a click to offer one option. The payload load stays automatic at boot and dismissible, which is where the operator meets it |
| D10 (2026-08-12) | §5.5 / §5.3.2: the device offers to ERASE the payload region | **the firmware never writes flash.** The operator may UNLOAD a payload — the session is cleared, flash is untouched — and the word *erase* is not used on the device | erasing implies the bytes are gone, and they are not. Overwriting the region stays a HOST operation (`me sysw wipe`). This also removes the only flash-write path the tree would have had, so no XIP-safety question and no destructive-write risk class arises at all |
| D11 (2026-08-12) | §3.2: every screen consuming a payload record names its source | the OFFER screen naming the source satisfies it where the operator picks — Engrave Text and BIP-39 Password render a separate F3 acceptance screen; the `syswOffer` sites do not, and need not | consistent with §3.2 as D5 already scoped it to the entry screen. Recorded so the asymmetry is a reading, not an oversight |
| D12 (2026-08-12) | §7's every-word verify inherits the seed keyboard's checksum gate | **`checksumGate` is OFF for §7 verification** | a checksum-masked keyboard hides a wrong LAST word, which is exactly the defect a mis-cut plate produces and §7 exists to hunt. The implementer's reasoning beat the plan's; absorbed here so it is not re-litigated |
| D13 (2026-08-12) | §12.2 `[compared]`: declining the digest comparison leaves the payload loaded-but-unconfirmed, and `take()` refuses it later | **declining UNLOADS it immediately.** `ctx.sysw = nil`, and the operator is told "Not Loaded — Digest not compared. Nothing was loaded." | operator ruling. A payload the machine holds but will not use is a state with no operator meaning: every later screen has to explain why the source it can see is refused. Unloading collapses that to one honest outcome at the moment the operator makes the decision. It costs nothing — the payload is still in flash and the loader can be re-entered — because per D10 the firmware never writes flash. **Implemented in fork `0d2801c`; CONFIRMED ON THE MACHINE 2026-08-13 at `v0.0.0-g20e99a6`** |
