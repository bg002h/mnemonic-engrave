...
- **Fork diff** (Tasks 1, 3): `CGO_ENABLED=0 go test -count=1 ./cmd/emu/`;
  ...
  the size recipe must read `1,581,204 B flash / 62,800 B RAM` -- fork `main`
  `70008da5` after W-4 (+624 B over W-3's 1,580,580, itself +640 over `60bee002`'s
  1,579,940; each step measured three times: implementer, verifier, controller);
  Tasks 1-3 add nothing, and a different number is a finding.
