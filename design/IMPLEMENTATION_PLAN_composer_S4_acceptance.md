...
**STATUS: TASKS 0-3 AND 6 SHIPPED 2026-09-03; TASKS 4-5 OPEN (the device).** Tasks 1-3 on fork `main` `6fb90cb` (merge of `composer-s4-emu`; whole-diff review `composer-S4-exec-review-r0.md` 0C/1I/4M/4N, folded and verified 8/8) and mnemonic-engrave `master` `e3ee51c9`; spec §12 items 2, 3 and 9 carry their EXECUTED lines; the journey is `design/journeys/SeedHammer-II-composer-journey.pdf`. Two device defects found on the way and shipped before any plate: W-2 (`3cc71d9b`, the pick lists were untouchable) and W-3 (`1ae0ffcb`, ids drawn under the buttons). Task 4 (the live walk from step 3 and ONE plate) runs after fork `main` is flashed at the operator's word; Task 5 at the operator's call. R0 record: GREEN 2026-09-03 (journey lens 1C/12I/5M/2N folded at `fda1d9e`; r1 fold verification 0C/0I/0M/0N).**
...
| 4 | CONFIRM → `Seat keys into this template?` rows `Engrave a key-less template`, `Type a seed` (drawn because there are NO sources, r0 I-4) | row 0 | consent `Review`: `Path 1: 2-of-3`; `KEY PATH: NONE (NUMS)` / `Spends use the script paths only. Bitcoin Core and Nunchuk import this form. Liana and BIP-388 signers need an unspendable xpub instead (see F-449).`; `Template-ID: e0863d3c...`; `mk1 stub (template): e0863d3c`; page 1: `Template has no keys - no addresses.` / `Verify off-device.` (shots per page -- the sentence CHANGED with F-531: this walk builds a template that DECLARES three slots and carries a key for none, which is `noAddressLines`' second arm, not the no-slots-at-all first arm it used to print) |
...
- **Fork diff** (Tasks 1, 3): `CGO_ENABLED=0 go test -count=1 ./cmd/emu/`;
  ...
  the size recipe must read `1,582,628 B flash / 62,800 B RAM` -- fork `main`
  `839fa5aa` after W-6/W-7 (+1,424 B over W-4's 1,581,204 at `70008da5`, itself
  +624 over W-3's 1,580,580 and +640 over `60bee002`'s 1,579,940; every step
  measured at least twice, by the controller and by an independent verifier --
  the W-6/W-7 round-2 verifier measured BOTH ends itself after round 1 took the
  baseline on trust); Tasks 1-3 add nothing, and a different number is a
  finding.
