...
2. **A kind-1 wallet inspected on the stage-3 device.** It shows no address (tested: the `stillUnsupported` entries, Task 5). It still shows the NUMS key-path line, because `policyShape` maps every non-Slot kind to `KeyPathNUMS` (`md/policy_shape.go:126`) until stage 4 adds §7's sibling kind. It is not false (the key is unspendable), but it is not specific either. **Not our concern at stage 3.** The stage 4 content already owns it.
...
- **`policyShape` reports kind 1 as `KeyPathNUMS`** until stage 4's §7 sibling kind. It is not false, but it is not specific either.
