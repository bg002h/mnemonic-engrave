...
sharpest finding is about this document's own discipline: **the archaeology pass
DELETED THE DELETION-DISCIPLINE RULE AND THE SAME COMMIT BROKE IT** (r9-I6). That
rule — *nothing is deleted without the fact landing somewhere and every citation
being repointed* — lived in the block being evicted; the pass promoted only its
sibling and then replaced a cited mechanism with an uncited, false one (r9-I7).
**Both rules are now §1.3's preamble, where the operation looks.**

**Three of round 9's four Criticals were SIBLING SITES of findings already
fixed** — C2's statement sat 89 lines below the row round 8 named, and C4 was the
row round 8 named *explicitly* while the fold corrected two others instead.
**Fixing exactly what a report points at is how this cycle keeps re-earning the
same defect**; every fix in this fold was applied at every site, and where a
string occurred twice both were changed (r9-M8).

**LINE COUNT: see `wc -l` — this header no longer states one.** v12's and v13's
figures were both wrong (r9-M1) for the same mechanical reason: the count was
measured, then written INTO the document, which changed it. A number that
invalidates itself on being recorded does not belong in the artifact it measures.
...
eviction is still outstanding and is the next operation.

**Measured delta v10 → v11: rules 20 → 13, vectors 30 → 24, table rows 156 → 146
— and LINES 2,007 → 2,068, which went the WRONG WAY.** Deleted rules and vectors
are struck **in place** with the reason, so the document grew while its scope
shrank. That is a deliberate choice for an artifact under active review and it is
...
71    8    fee         u64 BE          -- present iff bit0
 ..   4    fingerprint [u8;4]          -- present iff bit1
 ..   1+L  label       u8 len (1..=64) then L bytes UTF-8 -- present iff bit2
 ..   4    body_len    u32 BE
 ..   N    body        RAW: body_len bytes.  CHUNKS: body_len == 0 and the body is
                       EMPTY -- the chunks are SIBLING RECORDS (§1.4a)
```

> **(SIMPLIFICATION 3.1, 2026-08-25) THE LEGEND FIELDS WERE TLVs AND ARE NOW
> THREE FIXED OPTIONAL SLOTS BEHIND A FLAGS BYTE.** The three fields are in
> **fixed order**, each present iff its bit is set. Absence has exactly one
> spelling — the bit is clear — so there is no second way to say nothing.
>
> **The reason is that the plan had already disabled TLV's only benefit.** TLV
> buys forward compatibility: a later version adds a tag and older readers skip
> it. **E8 said *"An unknown tag is REFUSED, not skipped"*** — so a new field
> already required a coordinated upgrade of every implementation, and the plan
> was paying TLV's entire divergence surface for extensibility it had explicitly
> forbidden. A fourth legend field now costs a `version` bump, which is what it
> cost before.
>
> **DELETED OUTRIGHT: E1** (no order to violate — the order is structural),
> **E2** (a duplicate is inexpressible), **E8** (an unknown tag is
> inexpressible), **E10** (there is no `n_fields` to disagree with anything),
> **E16** (widths are structural). **E6 and E7 collapse into the flags byte**:
> "absent" is a clear bit and a zero-length value cannot be written. E4's
> Σ-arithmetic becomes a constant plus the label length. **Rules 20 → 14.**
>
> **Nothing is sunk and no ruling is touched.** Spec §8 — the operator rulings —
> contains **zero** mentions of TLV, `tag/len` or `n_fields`; §2.1b's heading is
> *"THE RECORD'S WIRE LAYOUT IS NOT DEFINED"* and delegates it here. `mt-codec`
> is unpublished, the sole producer (`mt`, P2) is unbuilt, and no shipped code
> encodes this record. **The cut pays three times** — Rust, the Go port (P3) and
> the device's legend read (P4) — because every TLV rule was a way two
> implementations could diverge, which is this plan's own definition of an
> E-rule.

**The framing is 75 bytes with all three slots ABSENT** — `4 + 1 + 1 + 32 + 32 + 1 + 4`, flags = 0.
**All three present with a 64-byte label it is 152** — `75 + 8 + 4 + 1 + 64`.
...
**(r8-I6) The label's length is a `u8`**, so 255 bytes is expressible, bounded only incidentally by
...
> **NORMATIVE FOR THIS DOCUMENT — TWO RULES, and (r9-I6) the archaeology pass
> DELETED THE SECOND ONE WHILE BREAKING IT.**
>
> 1. **No rule is added here without a stated input that would fail it, and that
>    input MEASURED.**
> 2. **Nothing is deleted without the fact landing somewhere and every citation
>    being repointed.** This governed the eviction itself. It lived in the v2
>    post-mortem block, the pass promoted only rule 1 out of that block, and the
>    same commit then replaced a cited mechanism with an uncited and false one
>    (r9-I7). **A discipline that can be deleted by the operation it governs has
>    to be stated where the operation looks**, which is here. Promoted here by the archaeology
...
| # | rule | why |
| --- | --- | --- |
| ~~E1~~ | ~~TLVs appear in ASCENDING TAG ORDER.~~ | **DELETED by SIMPLIFICATION 3.1 (2026-08-25)** — TLV order is gone; the three slots sit in fixed structural order and no order can be violated |
| ~~E2~~ | ~~A tag appears AT MOST ONCE. A duplicate is REFUSED.~~ | **DELETED by SIMPLIFICATION 3.1 (2026-08-25)** — a duplicate field is inexpressible — each slot has exactly one bit and one position |
...
| E4 | **`71 + S + 4 + body_len` MUST equal the decoded length exactly, where `S` is the sum of the PRESENT slot widths: `8` if bit0, `4` if bit1, `1 + label_len` if bit2.** | makes E3 checkable rather than aspirational. **The constant is 71** — `4 + 1 + 1 + 32 + 32 + 1`, the fixed part through the **flags byte**, which sits where `n_fields` used to. **(r8-C4) v11 left this row stating TLV's `Σ(3 + len)`** — three bytes of tag+len overhead per field — **which is 3 too many per present slot, so as written it REFUSED every legal record carrying a fee.** A Go porter transcribes this arithmetic, so it has to be the arithmetic of the format that exists |
...
| ~~E6~~ | ~~A zero-length TLV value is REFUSED.~~ | **DELETED by SIMPLIFICATION 3.1 (2026-08-25)** — collapsed into the flags byte with E7: a zero-length value cannot be written, because a slot is present or its bit is clear |
| ~~E7~~ | ~~An absent optional field is OMITTED from the list.** There is no empty encoding and no sentinel.~~ | **DELETED by SIMPLIFICATION 3.1 (2026-08-25)** — collapsed into the flags byte with E6: absence has exactly ONE spelling — the bit is clear — so there is no empty encoding and no sentinel to forbid |
| ~~E8~~ | ~~An unknown tag is REFUSED, not skipped.~~ | **DELETED by SIMPLIFICATION 3.1 (2026-08-25)** — there are no tags, so an unknown one cannot be written. **This rule is WHY 3.1 was taken**: it disabled TLV's only benefit (skip-what-you-don't-know) while the plan paid TLV's entire divergence surface |
...
| ~~E10~~ | ~~`n_fields` MUST equal the number of TLVs actually parsed.~~ | **DELETED by SIMPLIFICATION 3.1 (2026-08-25)** — there is no `n_fields` left to disagree with the number parsed |
...
| **E14** | **The LABEL slot's value (flags bit 2) MUST be valid UTF-8. Invalid is REFUSED.** | **(r1-I10; renamed r8-I6 — v11 still said *"Tag 0x01"*, and tag→bit is a PERMUTATION)** Rust refuses by default and Go accepts by default — one implementation engraves what the other rejects |
| **E15** | **The LABEL slot's length byte is `1..=64`.** | **(r1-I10; corrected r8-I6)** the length is a **`u8`**, which otherwise admits a **255**-byte label into a plate legend budgeted at 34 characters. **v11's cell still said `u16 len` and 65,535 — the TLV ceiling, which no longer exists** |
| ~~E16~~ | ~~A fixed-width tag MUST carry exactly its width: `0x02` → 8, `0x03` → 4. Any other `len` is REFUSED.~~ | **DELETED by SIMPLIFICATION 3.1 (2026-08-25)** — the widths are structural — `fee` is 8 bytes, `fingerprint` is 4, and neither is length-prefixed |
...
| **E20** | **Every `mt1` record SHOULD belong to exactly one CHUNKS `tx:` record's set, and every CHUNKS `tx:` record SHOULD have a COMPLETE set: `count` chunks, indices `0..count-1`, no gap and no duplicate.** | **(§1.4a; demoted MUST→SHOULD by RULING 2026-08-25)** the binding is R15's — `chunk_set_id` == the top 20 bits of that record's carried txid. **A violation is REPORTED, not refused** (§2.5a.2): the payload packs and engraves, because a partial set of independently-valid BCH-protected chunks is worth engraving and a `tx:` payload is not necessarily regenerable. `mt1`'s header is `version(5), chunk_set_id(20), count−1(15), index(15)` — 55 bits — so all three facts are read off each record without reassembling anything. Vectored by V25 |
| **E21** | **Bits 3–7 of the flags byte MUST be zero. A record with ANY reserved bit set is REFUSED.** | **(r8-C7)** v11 wrote *"bits 3-7 MUST be zero"* into the ASCII layout comment and gave it **no rule, no vector and no test** — measured, one mention in the whole document. That is round 0's **I3** exactly (*"a description of the encoder, not a refusal binding the decoder"*), reproduced inside the blockquote that quotes I3. Two implementers diverge on `flags = 0x09`: one refuses, one masks with `& 0x07` and engraves. **This rule is also the REPLACEMENT for the forward-compatibility surface 3.1 deleted** — E8 refused an unknown *tag*; E21 refuses an unknown *bit*, and the reasoning is identical. Vectored by **V6** |
...
**The exception is not a licence, and (r8-I1) THERE IS EXACTLY ONE.** **E11 is the only rule with no Rust RED test** — E7 was the other and 3.1 struck it outright, so the list got shorter by **deletion**, not by discharge. Every sentence that still says *"the two exceptions"* or *"EXCEPT E7 and E11"* is stale. It is named
...
`magic`, `version`, `form` pass; **every legend-slot rule passes**, because they
...
| --- | --- | --- |
| **the LIVE per-record rules and §2.3's RECORD-level decode failure** | **4** `EXIT_INVALID` | **(r8-C1) NOT E20**, and not any set-level condition: those REPORT at **exit 0** (§2.5a.2). v11 left this row spanning `E1–E20`, which includes the rule the same fold demoted to SHOULD it arrives through `pack → split → Err` and that path already returns 4 (`crates/me-cli/src/main.rs:983`). "Invalid/integrity" is what `me` calls this (`crates/me-cli/tests/cli.rs:162`)|
...
**So this section binds the LIVE PER-RECORD rules and §2.3's record-level failure — NOT E20, and not any set-level condition, which report at exit 0 (r9-I2; the row directly above already said so) — and the
...
| CHUNKS | gather the record's set (E20: `chunk_set_id` == the top 20 bits of the carried txid), require it COMPLETE and PRISTINE (E19), **reassemble via `mt_codec::decode`, THEN deserialise the result** as a Bitcoin transaction, **and check BOTH carried identifiers against it** — every record having satisfied E13 first. **(r8-C1) EVERY failure in this chain is a `SetFinding`, NOT an `Err`** (RULING 2026-08-25): an incomplete set is reported and packed, and so is a reassembly that is not a transaction. v11 left this row reading *"require it COMPLETE"* and *"require BOTH identifiers to match"* — the language an implementer turns into `Err` — after which V25, V27 and V28 all go RED with no code left to write. **(r6-C1) This chain is W15's**, and until round 6 it was owned by no site at all: §2.4 described E20 and stopped |
...
### 2.3 (I8; SPLIT BY RULING 2026-08-25) A `tx:` RECORD that fails DECODE is REFUSED — a chunk SET that fails is REPORTED
...
**NORMATIVE, and (r8-C1) IT SPLITS.** v11 folded the rulings into the sections
that NAME the conditions and left this one, which is where an implementer reads
the POSTURE. The two halves are not the same question:

| what failed | posture | why |
| --- | --- | --- |
| **a single `tx:` RECORD's parse** — magic, version, form, hex, `body_len`, a field, **and on the RAW form the identifiers** | **REFUSE**, `seal`'s posture (`crates/me-cli/src/seal/mod.rs:171`) | a malformed record is not partial material — the operator regenerates it by re-running `mt encode`, and nothing is lost by refusing |
| **a chunk SET** — incomplete, orphan, duplicate, `set_collision`, `identifier_mismatch`, `not_a_transaction` | **REPORT LOUDLY AND PACK** (§2.5a.2). **(r9-C3) The identifiers are on BOTH sides of this line**: on RAW they are the record's own bytes and refuse; on CHUNKS only the reassembled set decides them, so they report | the set carries a signing ceremony that may not be regenerable, and every chunk is independently valid and BCH-protected, so a partial set is worth engraving |

**The EPD §6.3 argument that justified refusing the whole class is ANSWERED
ELSEWHERE, not discarded.** v10 said warn-and-pack *"is not defensible for the
one record class whose failure mode is secret bytes riding in cleartext"*. That
is still the threat — but the container's own spec demoted the card-set decode
check to a **FLAG** on 2026-08-12 (`design/SPEC_systemwide_payloads.md:587`,
§5.3.2), and RULING 2026-08-25b puts the durable half on the **plate**: SH2
substitutes the legend, un-overridably. **The control moved from admission to
labelling; it did not go away.** §2.5a.2's ruling block states that trade in full
and this section is bound by it.
...
> **The table below names fifteen sites.** §2.4's own words applied to itself.

**NORMATIVE — FIFTEEN sites, and none of them is the codec.** **(r4-C1) W6 and
...
| **W12** | `crates/me-cli/src/sysw/mod.rs` | **(RULING 2026-08-25) A SET REPORT, NOT AN ERROR VARIANT.** v10 gave `SyswError` a set-level variant; nothing set-level errors any more, so the variant is **struck**. `split` returns `Vec<SetFinding>` alongside a **successful** pack — `{ set_id, condition, detail }` over §2.5a.2's five conditions — which is **`mdmk_unconfirmed`'s shape** (`crates/me-cli/src/sysw/record.rs:99`, returning `Vec<usize>`) with the set id and condition added, because five conditions need distinguishing where one did not |
| **W15** | `crates/me-cli/src/sysw/mod.rs` (with W10) | **(r6-C1) THE CHUNKS DECODE CHAIN — reassemble, deserialise, compare BOTH identifiers — now REPORTING, not refusing (RULING 2026-08-25b).** After a set is gathered, `split` reassembles via `mt_codec::decode`, deserialises the result as a Bitcoin transaction, and checks the carried txid and wtxid against it; **failure yields a `SetFinding`, not an `Err`.** **This is `mdmk_unconfirmed` with a different arbiter** — that function calls `md_codec::reassemble(&set).is_ok()` / `mk_codec::decode(&set).is_ok()` for the same purpose (`crates/me-cli/src/sysw/record.rs:156-158`), and P1 substitutes `mt_codec::decode` plus a transaction parse. v8 bound this chain to no site at all; v10 bound it to a refusal the container spec had already demoted |
| **W13** | `crates/me-cli/src/main.rs` | **the REPORT printer, not an error arm** (RULING 2026-08-25). Renders W12's `Vec<SetFinding>` to **stderr** after a successful pack — every missing index, never the first (r7-M1) — and `sysw_error`'s outer match keeps only the **per-record** arms (W8), which are the only failures left that abort |
| **W14** | `crates/me-cli/src/sysw/tx_vectors.rs` (new) + `crates/me-cli/testdata/tx_record_vectors.json` (new) | **(r4-I3, CORRECTED r5-C1, SCOPED r6-C2) the fixture loader and its conformance test.** Loads the `expect` sum type of §3.3 and asserts each vector's **CODEC** outcome — a `pass` arm against the parsed fields, a `refuse` arm against **the OUTCOME ONLY — no rule name** (3.2). **(r9-C4) Round 8's C5 named THIS ROW explicitly and the fold corrected §3.3 and §6's closure row instead, leaving the row an implementer actually builds the loader from byte-identical.** **NOT the packed blob, the exit code or stderr**: those need `me sysw pack` to accept a `tx:` record (W4, step 6) and a subprocess to observe, and this is a `src/` unit test built at step 4. They are `crates/me-cli/tests/sysw_cli.rs`'s, at steps 6 and 7. **NOT `coverage.rs`**: that is spec §8.3's placement table, `assert_every_named_test_is_placed` ends `assert_eq!(COVERAGE.len() as u32, 23)`, and P1 adds no §8.3 tests — so extending it **FAILS THE BUILD**, which is what v7's W14 row prescribed. Measured |

> **(r6-M7) WHERE THE `tx:` RECORD CODEC ITSELF LIVES — the one artifact v8 named
> nowhere.** §2.4's header says *"none of them is the codec"*, which is true and
> was read as *the codec needs no home*. **NORMATIVE: the parse and serialise
> functions live in `crates/me-cli/src/sysw/record.rs`**, beside `TxRecordError`
> (W11) and the existing `unhex_lower`. **This is forced, not chosen:**
> `unhex_lower` is **private** to that module
> (`crates/me-cli/src/sysw/record.rs:201`), so W4's branch as sited in `mod.rs`
> **cannot call it** — either the parse lives in `record.rs` and `classify` calls
> `record::parse_tx`, or a currently-private helper is widened for no other
> reason. The first is the repo's existing shape and keeps the hex decoder's
> `Zeroizing` return inside the module that owns it.
...
**These thirteen live sites are split across FOUR steps, not one (r4-C1, r4-C2;
count corrected r5-M1, r6-C1):**
**step 4** builds W14, **step 6** builds W1–W5 and W9, **step 7** builds W8 and
W11–W13, and **step 10** builds W10 **and W15 (r6-C1)**. **(r5-C2) W8 is step 7's, not step 6's** — it
is *defined as* the printer arm for W11's `TxRecordError`, and a Rust match arm
cannot name a type introduced by a later step. v7 drew this split by W-NUMBER and
the dependency runs the other way. **Step 6's test is END TO END** — `me sysw
...
vector in §3 can substitute for that test.** **(r3-M3, corrected r5-M2) EIGHT are not — the blockquote below re-derives them, and this sentence said FIVE — and siting
...
§6 states the per-site assertion for each of the **thirteen live sites** — W6 and W7
are retracted and are not sites at all (r5-I3) — because a `grep -c`
...
| **W12** | **(r8-C3 — SUPERSEDED; §2.4's row is authoritative)** v10 gave `SyswError` a set-level variant and **this summary still said so after §2.4 struck it.** An implementer building from *this* table adds `SyswError::SetLevel(..)`, and adding an error variant to what `split` returns is exactly how a set-level condition becomes an abort — **the refusal posture reinstated by following a NORMATIVE table.** W12 is a **REPORT**: `split` returns `Vec<SetFinding>` alongside a SUCCESSFUL pack. |
| **W13** | **(r8-C3)** the **report printer** for W12's `Vec<SetFinding>`, to stderr after a successful pack. `sysw_error`'s outer match keeps only the **per-record** arms — the only failures that still abort. §2.4's row is authoritative. |

#### 2.5a.1 (r6-I4; DEMOTED by SIMPLIFICATION 3.2, 2026-08-25) RULE IDENTITY IS RUST-INTERNAL

**v10 made the per-record rule name a three-hand cross-language wire contract** —
a Python generator emitting it, `TxRecordError`'s variants mapping to it, and the
Go port comparing against it — and then had to enumerate every name, keep three
artifacts in step, and defend the list against drift. **That contract produced
both of round 7's Criticals and three of its Importants**, every one a name that
no site could produce or that named the wrong record.

**NORMATIVE — the per-record rule name is a Rust-internal detail.**
`TxRecordError`'s variants are whatever the parse finds convenient. **The fixture
pins the OUTCOME, not the name:** `expect: {"pass": {...}}` or
`expect: {"refuse": {}}`. The Go port must agree on **which inputs are refused**,
which is the property that matters and the only one it could ever check
independently; **it need not agree on what the failure is called.**

**Two consequences, both good.** A rename is no longer a cross-language break, so
nobody has to keep three artifacts in step. And a name that no site can produce —
r7-C1's exact defect — becomes impossible to write down as a contract, because
there is no contract.

**The cost, stated plainly:** a negative vector no longer proves *which* rule
refused it, only that it was refused. That is acceptable **because P1's negative
vectors are single-axis by construction** (§3): each perturbs one field from a
known-good record, so "refused" localises the cause without the name. Where it
would not — a vector that could fail two ways — §3 already requires the
NORMATIVE construction clause that V8, V15 and V27 carry.

> **THE FIVE SET-LEVEL NAMES IN §2.5a.2 ARE NOT DEMOTED, and the rulings are why.**
> Under RULING 2026-08-25 every set-level condition is an **operator-facing report
> line** rather than an internal error, and §2.5a.2's whole point is that its five
> conditions have five *different remedies*. Those strings are read by a human, so
> they stay specified.
...
| condition | disposition | what the operator is told, and must DO |
| --- | --- | --- |
| **set INCOMPLETE** — a gap in `0..count-1` | **REPORT LOUDLY, PACK** | *"set `0x2dcf2` is INCOMPLETE — missing chunks 7, 12, 88 of 202"*. **Every missing index, never the first** (r7-M1). The operator engraves what they have and adds the rest later |
| **ORPHAN** — an `mt1` record matching no `tx:` record's set | **REPORT LOUDLY, PACK** | *"add the `tx:` metadata record this chunk belongs to"* — **and summarise, never abort on the first.** The plausible first mistake is packing `mt` chunk output alone, which makes EVERY chunk an orphan; a per-record abort would say *"record 0 is an orphan"*, whose obvious action is to delete record 0, then record 1, 202 times |
| **DUPLICATE INDEX** | **REPORT LOUDLY, PACK** | *"chunk 7 of set `0x2dcf2` appears twice"* — delete one copy |
| **`set_collision`** (R17) | **REPORT LOUDLY, PACK** | *"two chunk sets share their top 20 bits and cannot be told apart — pack these transactions as SEPARATE payloads"*. **Nothing else works: the txids cannot be changed.** One of only two messages here that changes what the operator does |
| **`identifier_mismatch`** — a COMPLETE set that reassembles to a VALID transaction whose txid or wtxid is **not** the one carried | **REPORT LOUDLY, PACK** | **(r9-C3)** v13 had two postures in three places for this: §2.2 said *"check BOTH carried identifiers"* and produced a finding, §2.3's table put identifier failures under **REFUSE**, and §2.5a.2 gave the resulting report **no name to carry**. It is a SET condition — the chunks decide it, not the record's own fields — so it reports. *"The chunks reassemble to a DIFFERENT transaction than the one this record names — re-encode from the source PSBT"* |
| **`not_a_transaction`** — a COMPLETE set that reassembles to non-transaction bytes | **REPORT LOUDLY, PACK — and the DEVICE substitutes the legend** | the §2.1 C3 smuggling channel. On SH2 the operator's chosen legend is **REPLACED, un-overridably**, by a warning to the effect of *"INCOMPLETE — RE-ENCODE PAYLOAD"*, and engraving proceeds |

> **RULING 2026-08-25 (a and b) — NOTHING IN THE CHUNK PATH REFUSES.** Both
> rulings are recorded verbatim in `design/FOLLOWUPS.md`. v10 refused on all five
> conditions; every one now reports.
>
> **This is not a weakening, and it is not even a divergence.** The container's own
> spec already says so — `design/SPEC_systemwide_payloads.md:587`, *"**5.3.2 The
> card-set DECODE check — now a FLAG, not a refusal** (demoted 2026-08-12, §13
> D6)"*: EPD §6.3's per-card-set decode requirement *"reaches this container as a
> **flag input**, not as the refusal EPD gives it."* **P1's refuse posture was the
> divergence — from the normative document it is built inside — and eight review
> rounds did not catch it, because every round compared the plan to itself.**
>
> **Why report beats refuse here, in the operator's own correction.** A `tx:`
> payload is **NOT necessarily regenerable**: the journey pipes `tx.final.psbt`, a
> FINALIZED transaction embodying a completed signing ceremony. A multisig quorum
> collected across time and geography may take days to re-collect or be impossible
> if a cosigner is gone; the source PSBT may already be lost; and **re-signing is
> not idempotent under this design** — a fresh nonce changes the signature and so
> the **wtxid**, which this plan carries (E17) and binds on (R15's top-20). A
> "regenerated" payload is a *different artifact by this design's own
> identifiers*. Refusing can cost the operator the ceremony; reporting cannot.
>
> **And a partial set is worth engraving**, because every `mt1` chunk is
> independently valid, BCH-protected, and carries its own `count` and `index` — so
> 201 engraved chunks plus the 202nd recovered later reassemble. That is precisely
> how `md1`/`mk1` multi-card backups already work.
>
> **LEGEND SUBSTITUTION IS THE DURABLE HALF, and it is the device's.** A stderr
> line is gone in a week and `me sysw show` must be re-run to be seen; **an
> engraved legend is permanent and travels with the plate**, on a device that has
> **no camera** and can therefore never read one back to warn anyone later. For a
> fifteen-year-recovery artifact that is the right surface for the warning.
>
> **THE DEVICE COMPUTES IT — it may not trust a host flag**, and this is the
> fork's existing shape rather than a new demand:
> `seedhammer/sysw/confirm.go:81` calls `md.Reassemble(set)` on-device and
> `seedhammer/seal/record.go:475`'s `decodePublicSet` enforces §6.3 there. A
> host-set flag byte would be worthless against EPD's stated adversary — *"a
> defective or third-party sealer"* — because a sealer that smuggles also sets the
> flag to "fine". **Cost: the fork has NO `mt` package and NO transaction
> deserialiser, so P3 must port `mt` reassembly AND tx deserialisation to TinyGo
> for `not_a_transaction`. Incomplete/orphan/duplicate are far cheaper — the chunk
> header alone decides them.**
>
> **TWO CONSEQUENCES, recorded so no future reviewer "fixes" them back.**
> **(1) The anti-smuggling gate is now LABELLING, not admission control** — the
> smuggled bytes reach metal, under a warning legend. The flash exposure EPD
> measured is unchanged (the payload is in flash once loaded); the plate is
> *additional* exposure a refusal would have prevented. Deliberate trade.
> **(2) Substitution is sometimes INSERTION** — the legend field is OPTIONAL, so a
> payload carrying none forces the device to ADD one, consuming plate space nobody
> budgeted. P4 owns that; P1 owes only that absence is representable, which it is.
...
**SEVEN operator-facing names in total, counted from the tables: 2 in the chunk channel (`chunk_charset`, `chunk_bch`) and 5 set-level (§2.5a.2).** v10 had **26**, because the per-record rule names were a cross-language contract; **3.2 demoted those to Rust-internal**, so they are no longer names anyone else must know. **A rule that gains a name later must be added HERE first** — the fixture, the enum
...
**The near-miss — and (r5-I4) v5's version of this paragraph was FALSE IN BOTH
CLAUSES once W6 was retracted.** It said a non-hex `tx:` body *"must still report
`NonHexBody`"*, because *"the existing channel gets it right"*, and *"it has a
test"*. Measured:

- **The existing channel never gets it right for `tx:`.** `unknown_reason`
  (`crates/me-cli/src/sysw/mod.rs:107-115`) iterates `[PASS_PREFIX, TEXT_PREFIX]`
  and nothing else; `grep -rn 'TX_PREFIX' crates/` returns **nothing**. **W6 was
  the only edit in this plan that would have made it emit `NonHexBody("tx:")`,
  and r4-C1 retracted W6** — so that value now has no producer, which is word for
  word the reasoning used to retract W7.
- **And it is unreachable by construction anyway.** W4 gives `classify` its own
  `TX_PREFIX` branch, so a `tx:` record never reaches `Class::Unknown` and
  `unknown_reason` is never called for one.
- **Its test was §6's W6 closure row, which r4-C1 deleted.** Nothing replaced it.

**NORMATIVE, replacing it:** a `tx:` record whose body is not lowercase hex is
**W11's**, and reports the **`hex` rule** through W8 — not `NonHexBody`, which is
the `text:`/`pass:` channel's value and stays theirs. **`NonHexBody("tx:")` is not
a value this plan ever produces**, and no step asserts it.
...
> in §4 step 4** (r6-M1: step 9 until r5-I5 moved it), which builds a synthetic 1-in/N-out transaction and serialises
...
| # | vector | pins |
| --- | --- | --- |
| V1 | **RAW (segwit)**, no optional slots | the fixed layout; **`flags = 0x00`** (r8-M3; v11 pinned `n_fields = 0`, a field that no longer exists — and V1 is the fixture's FIRST entry) |
...
| **V6** | **a reserved flag bit set — `flags = 0x09` (bit0 fee present AND bit3 set), followed by a valid 8-byte fee** → REFUSED | **(r8-C7, RE-CUT)** v10's V6 was *"unknown tag → REFUSED"*, TLV's forward-compatibility negative; 3.1 struck it and re-cut nothing, leaving the flags byte's five spare bits with no vector at all. **This is the same vector against the new encoding.** Its near-miss is `flags = 0x01` with the same fee, which PASSES — so the assertion is the reserved bit, not the fee |
...
| ~~V9~~ | ~~duplicate tag~~ | **DELETED by SIMPLIFICATION 3.1** — a duplicate field is inexpressible — one bit, one position (E2 struck) |
...
| **V12** | **label slot present (bit2 set) with `len = 0`** → REFUSED | **(RE-CUT by 3.1)** v10's V12 was *"zero-length TLV value"*, which is inexpressible now for `fee` and `fingerprint` — both are fixed-width with no length byte. **The label is the one slot that still carries a length**, so it is the one place a zero-length value can be written, and **E15**'s `1..=64` is what refuses it. Paired with **V22** (`len = 65`) and its `len = 64` near-miss |
...
| ~~V14~~ | ~~`n_fields` ≠ the TLVs present~~ | **DELETED by SIMPLIFICATION 3.1** — there is no `n_fields` (E10 struck) |
| **V15** | **R15 NEGATIVE: a chunks record whose carried txid's top 20 bits ≠ its chunks' embedded `chunk_set_id`** — **NORMATIVE: the perturbation goes into the CHUNKS' `set_id`, leaving the carried txid HONEST** | **(r8-C8) Under RULING 2026-08-25 this PACKS at exit 0** with two findings — every chunk is an **orphan**, and the `tx:` record's set is **empty/incomplete**. v11 rewrote V25/V27/V28 and left this row reasoning in refusals. **The RED test needs more than the outcome, and this is the whole point of the row:** delete R15's comparison and the chunks bind to nothing, which produces *the same two findings*, so an outcome-level assertion stays GREEN. **V15 therefore asserts that the orphan finding NAMES THE EXPECTED `set_id` — the carried txid's top 20 bits — beside the one found.** That string exists only if the comparison ran. **And it is a CONTAINER outcome, so V15 is filed in `crates/me-cli/tests/sysw_cli.rs`, NOT the codec fixture** (§3.3 forbids container outcomes there) — the same split r6-C2 drew and r7-I3 applied to V7's near-miss |
| ~~V16~~ | ~~TLVs in DESCENDING tag order** → REFUSED~~ | **DELETED by SIMPLIFICATION 3.1** — the three slots are in fixed structural order (E1 struck) |
| ~~V17~~ | ~~`tag=0x02, len=2`; and the near-misses `len=7` and `len=9` refused, `len=8` passes~~ | **DELETED by SIMPLIFICATION 3.1** — `fee` is 8 bytes structurally; no length accompanies it (E16 struck) |
| ~~V17b~~ | ~~`tag=0x03, len=3` and `len=5` REFUSED; `len=4` passes~~ | **DELETED by SIMPLIFICATION 3.1** — `fingerprint` is 4 bytes structurally; no length accompanies it (E16 struck) |
...
| **V21** | **the LABEL slot (bit 2) carrying `74 6f ff 21`** — not valid UTF-8 | E14. **(r8-I6)** v11 said tag 0x01, and **tag→bit is a PERMUTATION**: label → bit **2**, fee → bit **0**, fingerprint → bit **1**. A reader mapping 0x01 to bit 1 lands on the **fingerprint**, a 4-byte structural slot for which "invalid UTF-8" is meaningless |
| **V22** | **the LABEL slot with `len = 65`; near-miss `len = 64` PASSES** | E15. **(r8-I6)** v11 said tag 0x01 with len 65; see V21 on the permutation |
...
| **V25** | **E20's three conditions on one payload: (a) an ORPHAN `mt1` record whose `chunk_set_id` matches no `tx:` record; (b) a set MISSING an index; (c) a set with an index TWICE** | **(RULING 2026-08-25) all three PACK at exit 0 and each emits its own `SetFinding` on stderr** — v10 required all three to REFUSE. The assertion is the **report**, not the refusal: every missing index named, not the first, and the orphan line names *add the `tx:` record*, which is the remedy r7-M1 found inverts if omitted |
...
| **V27** | **two CHUNKS `tx:` records whose txids share their top 20 bits** | **(§1.4a, RULING 2026-08-25)** the collision R15-as-binding creates: 20 bits is *"1 in 1,048,576 by accident, and under a second to construct deliberately"*. **PACKS at exit 0 with a `set_collision` finding** naming the remedy — *pack these as SEPARATE payloads* — which is one of only two messages here that changes what the operator does. **NORMATIVE, because it decides whether the vector can go RED (cf. V15):** two colliding sets make every chunk match both `set_id`s, so the INCOMPLETE finding fires on its own; **V27 asserts the `set_collision` finding specifically**, and goes RED when only an incompleteness line appears. **spec §5's R17** |
| **V28** | **(r6-C1) CHUNKS whose set is COMPLETE and PRISTINE and reassembles to bytes that are NOT a transaction** — every chunk individually valid, `chunk_set_id` matching the carried txid's top 20 bits, so E13, E19, E20 and R15 all pass | **NORMATIVE, and it is the only vector that can go RED on W15**: delete the reassemble→deserialise→compare chain and every other vector stays green. This is the channel **§2.1 measured and named C3**. **(RULING 2026-08-25b) It PACKS at exit 0 with a `not_a_transaction` finding**, and the durable half is the DEVICE's: SH2 substitutes the operator's legend, un-overridably, with a re-encode warning and engraves anyway. **The host-side assertion is the finding; the legend substitution is P4's and is vectored there** |
...
### 3.3 (r4-I3, REWRITTEN r5-C1) WHERE THE TWENTY-FIVE VECTORS PHYSICALLY LIVE
...
`crates/me-cli/src/sysw/tx_vectors.rs`.** Not `sysw_vectors.json`, for the three
reason above (r9-M5; the archaeology pass compressed three reasons into one and left this pointer counting three).

**This follows the repo's existing precedent rather than inventing one.**
`crates/me-cli/testdata/` already holds **two** fixtures — `seal_vectors.json` and
`sysw_vectors.json` — one per container, each with its own loader. P1's records are
a third concern with a third schema need, and they get a third file.

**The cross-language guarantee is preserved, not traded away.** v7's argument for
the shared file was *"the Go implementation reads the SAME file"*. It still does —
**the Go port reads `tx_record_vectors.json`**, exactly as it reads the other two.
The guarantee comes from both sides reading one file, not from that file being
`sysw_vectors.json`.

**The schema MUST be able to hold a refusal, because most of these vectors are
refusals:**
...
{ "name": "V1-raw-roundtrip",
  "note": "the corpus even transaction, RAW form",
  "records": ["tx:4d5458..."],
  "expect": { "pass": { "form": "raw", "txid": "2dcf2b97…", "body_len": 222 } } }
```

> **NORMATIVE (r6-C2) — `expect` IS CODEC-LEVEL, and it may not name a container
> or process outcome.** v8's example carried `blob`, `pub_len`, `sealed` and
> `exit`: three are `sysw_vectors.json`'s container field names verbatim
> (`crates/me-cli/src/sysw/vectors.rs:29-40`) and `exit` is a **process** result.
> Asserting any of them requires `me sysw pack` to accept a `tx:` record — which
> it cannot until **W4 lands at step 6** — and requires observing a subprocess,
> which a `src/` unit test cannot do. **Step 4, which builds W14, forbids
> `me sysw pack` in its own loop.** So v8's schema made step 4 unfinishable, and
> the two ways out produce **materially different fixtures**: drop the container
> fields and the Go port gets a file pinning only the record codec; keep them and
> the loader must shell out from `src/`.
>
> **The split, ruled:** `tx_record_vectors.json` pins **what the record codec
> produces and refuses** — the parsed fields on `pass`, and on `refuse` **the
> OUTCOME ONLY, with no rule name** (SIMPLIFICATION 3.2; **v11 left this
> blockquote saying *"the rule name on `refuse`"* — r8-C5**). The cross-language
> contract is **which inputs are refused**, which is the property that matters
> and the only one the Go port could check independently; it need not agree on
> what the failure is called. **The container and process outcomes — exit
> code, stderr text, the packed blob — are asserted in
> `crates/me-cli/tests/sysw_cli.rs` at steps 6 and 7**, keyed to the same VECTOR NAMES (r9-M6; 3.2 removed the rule names this used to key on), where `me sysw pack` exists and a subprocess can be observed.

**`expect` is a sum type whose refusal arm is EMPTY** — the outcome, not the name
(3.2). **(r8-C5) v11 left this sentence claiming the arm carries a rule name and
that this is what lets V27 go RED. Both halves are false:** V27 no longer refuses
at all — it PACKS with a `set_collision` finding — and §3.3's own filing map puts
it in `tests/sysw_cli.rs`, not this fixture. **The five SET-level names are
operator-facing report strings (§2.5a.2) and live nowhere near `expect`.**

> **NORMATIVE — nothing regenerates this file from `me`.** There is no
> `regenerate` for it and none is written. It is produced by
> `scripts/gen-tx-record-vectors.py`, which **re-implements the framing
> independently**, exactly as `mt`'s own vectors come from `gen-mt1-vectors.py`
> (§3.2). A fixture the encoder can rewrite cannot falsify the encoder.

**W14 is the loader and its conformance test** (§2.4), **not `coverage.rs`** —
`COVERAGE` is spec §8.3's placement table and P1 adds no §8.3 tests, so touching it
is the defect rather than the gate.

**Which step files which vector.** §4 **step 4** writes and commits
`scripts/gen-tx-record-vectors.py` and files **V1–V5, V8, V10–V13, V18, V19, V21, V22, V26** — **(r9-M8) NOT V20, V23 or V24**, which are bare `mt1` records the layout codec never sees (step 8's own cell says so) and which step 6 asserts through W5 — — the live RECORD-level rows, **19 plan ROWS (r8-I4; v11 said 27 and re-asserted five struck vectors inside a range)** — (**and (r6-M5) MORE than 19 fixture ENTRIES**: a row naming a refusal and its near-miss is two, and V13 alone is three (`magic`, `version`, `form`). §6's near-miss bullet enumerates the pairs and is what the entry count must satisfy,
counting V4a and V4b; **V17b is struck by 3.1**). **(r9-I4) The map must account for all TWENTY-FIVE live vectors: 19 record-level at step 4, V6 at step 8, V7 at step 9, V15/V25/V27/V28 in `tests/sysw_cli.rs` as container outcomes.** **Step 9** files **V7** — **but (r7-I3) its `16,291 − F` NEAR-MISS goes to
...
> **NORMATIVE (r6-M6) — every test that packs a `tx:` or bare `mt1` record uses
> `--in` or stdin, NEVER argv.** 30 of the 31 tests in
> `crates/me-cli/tests/sysw_cli.rs` pass records on argv, so a test written the
> file's own local convention is **refused by R2 the moment step 11 lands** —
> steps 6, 7 and 10 would each go RED three steps after closing green, violating
> the rule directly above. Both channels exist from step 2.
...
| **7** | **a `tx:` record that fails DECODE aborts the whole pack: nothing on stdout, one stderr line naming the index and the rule, exit 4 — with four other valid records present. AND (r4-C1) a record with magic `MTX2` and a VALID lowercase-hex body names the MAGIC rule — the string *"not lowercase hex"* MUST NOT appear. **(r5-C2, moved from step 6) a `tx:` record whose body is NOT lowercase hex names the `hex` RULE, not `Unrecognised` and not `NonHexBody` (§2.5a)** | **§2.3's refusal, at the layer and scope §1.5 rules — and THE ERROR PATH: W11, W12 and W13 of §2.4 (§2.5a).** The anti-smuggling gate, tested as a `me sysw pack` outcome rather than as a codec `Err`. **The second clause is the one v4's RETRACTED W6 could not pass** |
| 8 | **V5, V6, V10–V13, V19, V21, V22** — **(r9-C1) V6 IS LIVE AGAIN**: r8-C7 re-cut it from *"unknown tag"* to *"a reserved flag bit set"* and **both steps that could file it still called it struck**, so E21 — a live REFUSAL rule — was built by nothing — **(r8-I3) NOT V15**: v11 added it here, but V15 is a *container* outcome (it packs, emitting two findings — r8-C8) and is unbuildable at the codec layer. It is **step 10's** — **(SIMPLIFICATION 3.1) V6, V9, V14, V16, V17 and V17b are STRUCK**: each tested a TLV property that no longer exists — unknown tag, duplicate tag, `n_fields` disagreement, tag order, tag widths. **V12 is RE-CUT** to the label slot, the one field that still carries a length — **(r4-I5) V20, V23 and V24 are NOT here: they are BARE `mt1` records with no `tx:` framing, and the layout codec never sees one.** They move to **step 6**, where W5's `classify` branch is what refuses them; sited here they would pass vacuously against a codec whose input is a framed record | **every LIVE rule this step's vectors reach — E3, E4, E5, E9, E14, E15, E18 — and no others.** **(r8-I2) v11's column claimed `E1–E19 EXCEPT …`, which asserts coverage of six rules 3.1 had struck and cites E7 as an exception when E7 no longer exists.** The live set is thirteen (§1.3); E11 has no Rust RED test, E12 is step 12's, E13/E19 are step 6's, E17 is step 5's, E20/E21 are elsewhere — **(r4-M1, re-derived r5-I2 against THIS fold's siting)** **E7 and E11** are §1.3's two NORMATIVE exceptions with no Rust RED test at all, and v7 DELETED that carve-out while §1.3 and §6 both still carry it; **E13's vectors (V20, V23) and E19's (V24) were moved to step 6 by this very fold**, leaving both named by a rule column with no vector under it; E12's RED test is **step 12's**, E17's is **step 5's** (*"V18 and V26 belong here and not in step 8"* — step 5's own words), and E20 is **step 10's** — and **outside the range `E1–E19` altogether**, so v7's *"E1–E19 EXCEPT … E20"* excepted a non-member (r5-I2) — because it is a payload-level rule and its vectors are payloads |
| 9 | V7 at **16,290 − F**, and the near-miss `16,291 − F` REFUSED | the framing ceiling, single-record (§3.1) — **and this is the step that USES `scripts/gen-tx-record-vectors.py` to emit V7 and its near-miss** — **(r6-M1) step 4 WRITES and commits it**; r5-I5 moved it there and left this cell and §3's blockquote saying step 9 — because V7 has no other input |
| 10 | V8, **V15**, **V25**, **V27**, **V28 (r6-C1)** — **and (r4-C2) the two halves steps 4 and 5 cannot hold: V3's WHOLE-PAYLOAD round-trip and V4b's R15-positive binding.** Both need `split`'s set pass, which is this step | the identifier-consistency checks on both forms — **refusals on the RAW form, `SetFinding`s on the CHUNKS form (r8-C1)** — **E20's set binding** (§1.4a), **and W15's CHUNKS decode chain — reassemble, deserialise, compare both identifiers (r6-C1), whose only RED test is V28**. **This is the first step at which a payload containing bare `mt1` records can be packed and read back at all**, because W5 (step 6) and W10 (here) are both in place |
| 11 | a `tx:` record on **argv** refused (R2) **at EXIT 3**, **and its text does not appear in stderr** — **and (r6-I3) SO IS A BARE `mt1` RECORD.** R2's rationale is *"argv is world-readable via `/proc` and lands in shell history; this material is bearer"*, and **§1.4a moved the transaction OUT of the `tx:` record**: for the CHUNKS form that record carries an empty body and the bearer material is the 202 `mt1` siblings. A prefix-shaped guard would refuse the empty envelope and wave through everything it was protecting. **`Class::MtChunk` on argv is refused at EXIT 3 on the same grounds**, and spec §5's R2 — *"a `tx:` record on argv"*, singular — is one of the statements §6.3 owes a correction | the argv guard — §1.5's *what runs before it*: clap must not echo the record. **Exit 3, not 4** (r2-I2): it is a policy refusal, the shape of `crates/me-cli/src/main.rs:509-515` |
| **12** | **(r9-M7) `me sysw show` on a PARTIAL set prints `INCOMPLETE` with every missing index** — RULING 2026-08-25's second normative MUST, which r8-C6 wrote into W9 and no step asserted. **AND (r3-I1) E12's RED test, in `sysw`** — pack a CHUNKS `tx:` set, read the public section back as bytes, and assert it is the records joined by a single `0x0A` with **no trailing LF and no empty record**. It goes RED when `crates/me-cli/src/sysw/mod.rs:260`'s separator changes | E12, which until this step is a rule the plan states and nothing in `sysw` asserts. **Not** `seal`'s `joins_with_lf_and_no_trailing_lf`, which tests a different container |
...
- **all TWENTY-FOUR live vectors pass** (r8-M4; v11's *"V1–V27"* named six struck vectors and omitted V28, W15's only RED test), and **V4a/V4b write out txid, wtxid and `chunk_set_id`
  explicitly** in the vector file, not only in code.
- **Every LIVE rule has a test that goes RED without its check.** **(r9-I1) E21 is one of them** — added by r8-C7, vectored by V6, filed at step 8 — so the live set is **fourteen**, not thirteen, and any sentence below counting thirteen or ending the numbering at E20 is stale. The numbering still runs E1–E20 because numbers were not reused, but **SIMPLIFICATION 3.1 struck seven — E1, E2, E6, E7, E8, E10, E16 — leaving THIRTEEN**, and the exception list below is now shorter by two: **E7 was one of the two, and it is struck outright.** The remaining exception is E11 alone — **EXCEPT E7 and
...
- **`ClassTransaction` and `ClassMtChunk` are WIRED, not merely specified**
  ...
  | **W14** | **(r5-C1, scoped r6-C2, corrected r8-C5)** every vector in `tx_record_vectors.json` is asserted by `tx_vectors.rs` at the CODEC layer. **The mutation that defines this gate is FLIPPING AN EXPECTED OUTCOME — change one `refuse` to `pass` (or the reverse) and the test goes RED.** v11's row said *"flip any expected rule name"*, **which cannot be done once the arm is empty — the gate was unrunnable** — flip any expected rule name in the fixture and the test goes RED. **v7's row (*"both assertions pass with P1's 29 vectors present"*) PASSED WITH ZERO PRESENT** and is struck: satisfiable without doing the work, the false-PASS shape §6 struck under r2-I4 |
  ...
  | **W12** | a payload with chunks 7, 12 and 88 of set `0x2dcf2` missing **PACKS at exit 0** and its `SetFinding` names **the set and ALL THREE indices** — not the first, and not a bare record index, which E20 does not have (RULING 2026-08-25) |
  | **W13** | every `SetFinding` reaches **stderr after a successful pack**, and `sysw_error`'s outer match keeps only the per-record arms; nothing prints a bare `Unclassifiable` |
  ...
  | W9 | `me sysw show` on a packed `tx:` payload prints the carried txid **and the carried wtxid** (r3-I4), **and on a partial set prints `INCOMPLETE` with every missing index (r8-C6)**, and on a 202-chunk payload prints **one** set line. **(r4-I2) the assertion is NOT *"V18 and V26 differ in 32 positions"* — v6 wrote that and it CANNOT RUN, because V18 is a vector E17 REFUSES, so `show` never emits a line for it.** The runnable assertion, which delivers the same operator benefit: on the honest 222-byte corpus record the printed **txid and wtxid DIFFER**; on **V26** — which the plan requires to PASS — they are **EQUAL**, because a witness-free body's wtxid is its own txid. That equality is the signal that the payload carries no signatures, and it is visible in `show`'s output or it is visible nowhere |
  | **W10** | **(r8-C2)** V25's three conditions each **PACK at exit 0** and each emits its own `SetFinding`; V3's complete payload packs with **NO** finding. **v11 left this row reading *"V25's three negatives all refuse"*, the exact negation of the V25 row the same fold wrote** — and this is a CLOSE CONDITION, so implementing the ruling made the gate unsatisfiable while satisfying the gate made V25 red. The `SetFinding` count is the assertion: **three for V25, zero for V3** |
  | **W15** | **(r6-C1, RULING 2026-08-25b)** **V28 PACKS at exit 0 carrying a `not_a_transaction` finding** — a complete, pristine, correctly-bound chunk set whose reassembly is not a transaction. **Delete W15's chain and V28 is the only test in the suite that goes RED**; v8's W10 row (*"V25 refuses, V3 packs"*) is satisfied by an E20-only implementation and could not detect this |
...
- **The vectors were not produced by the code they judge** (§3.2) — **(r5-I1)
  v7's version of this bullet was a FALSE PASS, and §3.3 said so in the same
  fold while leaving it standing.** *"Satisfied by construction, since §3's
  transaction comes from `gen-mt1-vectors.py`"* is **true no matter how the
  framing was built**: the corpus supplies the *transaction*, while magic,
  version, form, both identifiers, the flags byte, the present slots and `body_len` come from
  somewhere the bullet never names. It could not detect the defect it exists to
  detect. It also claimed V27, which §3's exception says needs a **second,
  ground** transaction no corpus supplies.

  **NORMATIVE, replacing it — and it can FAIL:** every vector in
  `crates/me-cli/testdata/tx_record_vectors.json` is emitted by
  `scripts/gen-tx-record-vectors.py` (§4 step 4), which **re-implements the
  framing independently of `me`**, and **no `regenerate` path exists for that
  file** (§3.3). The check is `scripts/gen-tx-record-vectors.py` reproducing the
  committed fixture **byte for byte from a clean checkout** — change one framing
  byte in `me` and the fixture is unaffected, which is the whole point; change it
  in the generator and the diff is visible. **v7's compensator was *"the
  `regenerate` command was NOT run"*, an ABSENCE** — and §6 itself rules twenty
  lines above that *"an absence is not assertable… each site gets a test instead,
...
- **The near-miss rule**: every guard here is tested against its nearest
  *legitimate* input as well as the hostile one — **(M3) v2 said "seven instances
  ...
  chunk refused / the pristine one passes)**, **V25 (an incomplete set REPORTED-AND-PACKED, its finding
  naming EVERY missing index / V3's complete set packs with NO finding — **(r9-C2)
  v13 still said *"an incomplete set refused"* here, 89 lines below the W10 row
  round 8 named as its C2 and this fold rewrote: same statement, different site,
  only the named one fixed**)**, V10 (trailing bytes refused / exact end passes),
  **V7 (`16,291 − F` refused / `16,290 − F` passes)**, **V6 (`flags = 0x09` refused / `flags = 0x01` with the same fee passes — r9-M3, E21's pair, omitted by v13)**, and §4.2's TTY refusal
...
only when an anchor comment names a file on a **hard-coded allowlist** —
`p.startswith("src/seal/") or p == "tests/seal_cli.rs"`
(`scripts/plan-build-gate.sh:71`) — and **this plan
...
and the fold author is the only one who knows what was superseded.** These **fifty-three**,
...
'extend `crates/me-cli/testdata/sysw_vectors.json`'
                               the WRONG vector home            (r5-C1)
'the existing channel gets right'
                               NonHexBody has no producer       (r5-I4)
'W1–W5 and W8–W9'              W8 is step 7's                   (r5-C2)
'each of the **fourteen**'     twelve LIVE sites                (r5-I3)
'THREE steps, not one'         four steps                       (r5-M1)
'twenty-two below'             twenty-seven                     (r5-M3)
'sixth enumeration'            eight, and the tally miscounted  (r5-M4)
'E1–E19 EXCEPT E12, E17 and E20'
                               E7/E11/E13/E19 too               (r5-I2)
'FOURTEEN sites'               fifteen rows, thirteen live      (r6-C1)
'the rule name on stderr and the exit code'
                               W14 is CODEC-level               (r6-C2)
...
'Tag `0x01`'                   tag->bit is a PERMUTATION        (r8-I6)
"follows `seal`'s posture — refuse"
                               2.3 SPLITS record from set       (r8-C1)
'TWENTY-SIX rule names'        per-record names went internal   (3.2)
'V1–V27 pass'                  24 live vectors, and V28         (r8-M4)
'the twenty-nine'              twenty-four                      (r8-M2)
'a file it can assemble'       a hard-coded seal allowlist      (r9-I7)
'reasons above.'               the pass compressed three to one (r9-M5)
'V18–V24, V26'                 V20/V23/V24 are step 6's         (r9-M8)
'arm against **the rule name**'
                               outcome only (3.2)               (r9-C4)
```

> **(r9-I5) AND v13 ADDED THE PREVIOUS FOLD'S TERMS, NOT ITS OWN** — the same
> failure one round later, which is why four r9 terms are appended below.
> `'an incomplete set refused'` is deliberately NOT among them: this document
> quotes it while explaining r9-C2, so listing it would fire the gate on the
> correction — the seventh instance of that recursion in this cycle.
>
> **(r8-I5) v11 ADDED NOT ONE TERM while claiming *"each of which this fold
> removed"*, so §6.1's close condition was satisfied trivially — the gate was
> handed an empty question.** Round 8 ran it with ten terms v11 actually
> superseded: **ten of ten were still standing**, and each one was C1–C5, I1 or
> I4 of that report. **The gate works; nobody pointed it at the fold.**
>
> **Only FIVE of v11's terms are listed below, and the omission is deliberate.**
> The other five — `Σ(3 + len)`, `set-level variant`, ``rule name on `refuse` ``,
> `V25's three negatives all refuse`, `n_fields = 0` — are **quoted in this
> document while explaining the fix that removed them**, so listing them would
> fire the gate on the corrections. Same reason §6.3's tally DESCRIBES its stale
> phrases instead of quoting them; **this is the sixth time in this cycle that
> documenting a retraction re-created it.**

**The last FOUR are r9's, the five before them r8's, the four before them r7's, the five before those r6's,
the eight before those r5's, the five before those r4's, the ten before those
r3's**, each a fact its fold RETRACTED. A hit for any of the fifty-three **outside the block above** is a real finding — that is the
...
| ~~**R17**~~ — two chunk SETS sharing top-20 bits | ~~§4 step 10~~ | ~~**4**~~ | **STRUCK (RULING 2026-08-25)** — R17 no longer REFUSES, it reports, and this table is REFUSAL coverage. Its assertion lives in V27's row and §6's W12 row. Struck rather than deleted so a reader who remembers it from v10 finds out what happened |
...
> **THE RUNNING TALLY, and (r5-M4) it had itself become one of the entries it
> counts.** v6 wrote *"the sixth"* above four examples and v7 changed two of them
> underneath it without renumbering. Re-derived, every one measured:
>
> | # | claimed | actual |
> | --- | --- | --- |
> | 1 | three lockstep sites | four |
> | 2 | five wiring sites | ten |
> | 3 | ten wiring sites | thirteen, then **fourteen rows / twelve live** |
> | 4 | three stale spec statements | five |
> | 5 | §2.4's record-level vector residue | **29 rows, 8 exceptions, so 21** |
> | 6 | §2.4's count of steps the sites split across | **four** |
> | 7 | §2.4's per-site assertion count | **twelve live, then thirteen (r6-C1)** |
> | 9 | §2.4's site total, and §3's vector total | **fifteen rows / thirteen live; thirty vectors (r6-C1)** |
> | 8 | §6.1's fold-sweep invocation term count | **twenty-seven, then thirty-five** |
>
> **NINE, not six** — and entries 5 through 8 were all written by folds whose own
> purpose was to correct a miscount. **This table DESCRIBES each stale phrase
> rather than QUOTING it, deliberately**: the first draft quoted them, and three
> re-entered §6.1's sweep as fresh hits outside the block. That is the fourth
> time this cycle that documenting a retraction re-created it — the same shape as
> quoting a bad citation and minting it again. **An enumeration in this document is a
> hypothesis until something re-derives it**, and that now includes this table.
