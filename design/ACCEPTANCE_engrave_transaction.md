...
| ~~R3~~ | **RETRACTED 2026-08-25** | the rule required `mt encode` to name a record form because two existed. `--chunks` was then measured a **byte-for-byte no-op** — identical to bare `mt encode` on stdout and stderr — because the 2026-08-24 bare-records ruling deleted the wrapper it existed to build. A choice with one side is not a choice: the family collapsed to `--qr`, `record_form_guard` is deleted, and the three dead flags are unknown to the parser (`the_retired_record_family_is_unknown_to_the_parser`). `mnemonic-transaction` `1282260`. |
...
| `mt` emits the record, `me` packs | **MET (P3b)** | `mt encode --qr` emits the `tx:` record and bare `mt encode` emits the bare `mt1` records; `me` packs both and manufactures neither. **`me tx` is deleted** — `me` had no other verb that produced a constellation string, and the one it had disagreed with the consumer one step downstream (G-P3.19) |
...
| ~~no `--record` default; the refusal teaches~~ | **RETRACTED 2026-08-25** | R3 — the pair collapsed to `--qr` |
...
**Re-measured after P3b** (the `me tx` → `mt encode` graft), by the
same script. Three verdicts moved and one gate closed: R3 and the two
record-form rows became **MET** because the flag then existed to have no default,
