...
- **Status:** R0 GREEN (0C/0I) — opus architect review `a07f08aed3b2a9615` (round 0), persisted at `design/agent-reports/seedhammer-engrave-bugfixes-spec-R0-round0.md`; 3 non-blocking Minors folded (additive). Cleared for the implementation-plan phase (which gets its own R0 gate).
...
2. **Correct-safe-point assertion:** for a leading-move-knot plan, assert the selected `sp.safePoint` matches an independently-computed reference: the control point of the most recent fully-elapsed clamped triple at the given `completed` level (NOT merely "the wrap didn't happen"). **Closed form (R0-folded for implementer clarity):** the reference safe point is the `k0.Ctrl` of the last clamped triple `(k0,k1,k2)` (`k0.Ctrl==k1.Ctrl && k1.Ctrl==k2.Ctrl`) all of whose cumulative `T` sums to **≤ `completed` reported ticks**; the assertion must verify it **never selects a triple whose start exceeds `completed`** (i.e. never a not-yet-reached safe point). Construct the plan so a clamped triple exists and verify the safe point advances to it only after enough progress is reported.
...
> **\*N=25 is illustrative/unreachable (R0-folded):** no GUI path emits 25 words — `slip39LengthPick` offers `{20,23,27,30,33}` and BIP-39/BIP-85 offer only `{12,18,24}`. The reachable `N>24` counts that actually exercise the A1 path are **{27,30,33}**. N=25 is shown only to exhibit the formula at a small over-24 count.
...
> **Deliberate change for N=27 and N=30 (R0-folded — NOT a regression):** the single `N>24` predicate intentionally moves the *already-correct* N=27 and N=30 layouts from the legacy two-block col-2 split to the rebalanced single-block A1 split (both stay at the full 4.1mm font; e.g. N=30 col-1 changes [9.70,75.30]→[11.75,73.25]). This is by design (one uniform large-N rule removes the two-block collision mechanism entirely rather than special-casing N=33) and is ratified at R0 as acceptable-as-designed. The post-implementation exec review MUST treat the N=27/30 golden *change* as expected, not as a regression — only N≤24 and QR plates are required to be byte-identical.
...
2. **Regression pin — N∈{20,23,24,30} + a QR plate:** golden/coordinate snapshots that the {20,23,24}-word and a QR-bearing (BIP-39, `genSeed` with QR) plate are **byte-identical** to current output (extend the existing golden mechanism, `compareGolden`, which already pins `TestSeed` 12/24 and `TestSLIP39` 20). For **N=30** assert no-overlap + within-bounds (it newly takes the `N>24` path at full font 4.1mm — pin its y-ranges to the table). These guard the #1 risk (regressing the shared ≤24+QR path) and confirm N=30 didn't change font.
   - **Golden provenance (R0-folded):** the *committed* zero-churn guards today are `seed-*-words-{12,24}` (BIP-39 + QR) and `slip39-0` (20-word). There is **no committed golden for 23/24-word SLIP-39**, so a "byte-identical" pin for N=23 is a **freshly-generated** pin — it asserts the new code leaves the *untouched* ≤24 path producing the legacy formula's output (valid because N=23≤24 never enters the `N>24` branch), not a diff against a pre-existing file. Generate the N=23 reference from `f907eea` (or compute the legacy formula) and pin it.
