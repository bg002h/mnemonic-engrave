...
- **VF6 — `PathComponent` ALREADY EXISTS** (exported) in `md/encode_singlesig.go:20` (the `type PathComponent struct {` line; `:16`–`:19` is its doc-comment) as `type PathComponent struct { Hardened bool; Value uint32 }` — RAW form (Hardened flag + bare value, NOT in-band `+HardenedKeyStart`). REUSE it; do NOT redefine.
...
> **Which test carries byte-correctness vs which is a drift-guard (R0-M2 framing, so a reader does not mistake the self-referential golden for the correctness anchor):** **A2** (the full-policy chunk-string goldens, Task 3b/3d) is a **drift-guard / frozen-output regression** — it freezes the assembler's OWN output and re-asserts it, so on its own it is self-referential and proves nothing about byte-correctness. The **byte-correctness anchors are A1 and A3**: **A1** (Task 3a) is template-parity against the Rust-sourced `wsh_sortedmulti.bytes.hex` / `sh_wsh_multi.bytes.hex` (independently generated by the Rust encoder), and **A3** (Task 3c) is byte-exact equality against the independently-vendored T6b fixture (`gui/testdata/t6b_multisig_full.md1.txt`, commit `e1c4240`). A1+A3 are the non-circular gates; A2 only catches future drift in the frozen output.
...
Create `md/encode_multisig_test.go`. **Imports accrete per task** (R0-I2): each task's
test step imports ONLY what its body uses, so every per-task commit compiles
green in isolation. Task 1 uses only `testing` (the new in-package types need no
import). Later tasks ADD imports as their using-code lands: Task 2 adds
`encoding/hex` + `seedhammer.com/codex32`; Task 3 adds `encoding/json`, `os`,
`path/filepath`, `strings`; Task 5 adds `errors`. The final all-tasks file uses
every import.
...
Run: `go test ./md/ -run TestEncodeMultisigRequestPlumbing -v`
Expected: PASS. The test file imports ONLY `testing` at this task, so it COMPILES
clean (no "imported and not used") and the Task-1 commit builds green in isolation.
The other imports (`encoding/hex`, `errors`, `os`, `path/filepath`, `strings`,
`encoding/json`, `seedhammer.com/codex32`) are added in the later tasks that first
use them.
...
First, EXTEND the import block (accretion, R0-I2): Task 2's body uses `encoding/hex`
(in `mkXpub65`) and `seedhammer.com/codex32` (in `codex32.ValidMD`), so widen the
single-line import to:
...
Then append to `md/encode_multisig_test.go`:
...
- [ ] **Step 2: Run the test — it PASSES immediately for both vectors**

Run: `go test ./md/ -run TestEncodeMultisigTemplateParity -v`
Expected: PASS for BOTH sub-tests (`wsh_sortedmulti` and `sh_wsh_multi`) — this exercises the SHIPPED `encodePayload` against the existing vendored goldens with hand-built trees built at each vector's OWN n (n=3 / n=2). It is a parity guard that documents VF2/VF10; no new production code. The pinned `wantHex` values are R0-verified: `wsh_sortedmulti` → `2082001821c22180`, `sh_wsh_multi` → `2042001830860850`. (VF2/VF10 are correct — there is no "re-verify VF2" failure mode here; a mismatch would indicate the hand-built tree or the vendored `.bytes.hex` was altered, NOT a wire-core defect.)
...
```
…and EXTEND the import block (accretion, R0-I2): Task 3's loaders/parsers first use
`encoding/json` (`jsonUnmarshalStrict`), `os` (`os.ReadFile`), `path/filepath`
(`filepath.Join`), and `strings` (`strings.Split`/`TrimSpace`/`NewReader`). After
this task the import block is:
...
```
(`errors` is still NOT imported until Task 5.)
...
First, EXTEND the import block (final accretion, R0-I2): Task 5's body uses
`errors` (in `errors.Is`). The import block reaches its final all-tasks shape:
