...
| 1.4 | `text:` / `pass:` bodies are **lowercase hex** | EPD §6.6 hashes records in canonical LOWERCASE form. Hex is the only common encoding that survives lowercasing unchanged — base64 and base32 do not. Cost is 2×, against a cap of 32,734 (8191 before the transaction-engraving cycle raised it), so nothing real binds. |
